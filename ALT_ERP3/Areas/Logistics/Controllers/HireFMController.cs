...
{
    public class HireFMController : BaseController
    {
        ...
        private static List<SelectListItem> PopulateBranches()
        {
            ...
            using (SqlConnection con = new SqlConnection(HireFMController.connstring))
            {
        ...
        public ActionResult CheckFMDate(string DocDate, string DocTime)
        {
            ...
            if (Status == "T")
            {
                ...
                if (ctxTFAT.PeriodLock.Where(x => x.Branch == mbranchcode && x.Type == "FMH00" && x.LockDate == NewDate).FirstOrDefault() != null)
                {
                    Status = "F";
        ...
        {
            string DriverSpclRemark = "", DriverBlackListRemark = "";
            bool DriverHireSpcl = false, DriverHireBlackList = false;
            string VehicleNoName = "";
            string HireSpclRemark = "", HireBlackListRemark = "", MaintainDriverAc = "F", FMVouRel = "F", CatchFreight = "F", DriverNCombo = "", DriverContact = "", DriverCode = "", Payload = "", BroCode = "", BroNAme = "", VehicleCategoryCode = "", VehicleCategoryName = "", OwnerName = "", DriverName = "", LicenNo = "", LicenceExpDate = "", VehicleGroupStatusCode = "", VehicleGroupStatusName = "";
            int KM = 0;
            bool CutTDS = false, HireSpcl = false, HireBlackList = false, AllowToChange = false;
            decimal TDSRate = 0;
            ...
            if (!(String.IsNullOrEmpty(Vehicle)) && !(String.IsNullOrEmpty(FromBranch)) && !(String.IsNullOrEmpty(ToBranch)))
            {
                ...
                if (vehicleMaster != null)
                {
                    if (vehicleMaster.PickVehicleRate)
                    {
                        ...
                        if (vehicleMaster.PostReq == true && vehicleMaster.MaintainDriverAC == true)
                        {
                            //Catch Freight And Advance Separately
                            ...
                            {
                                //From TO
                                Freight = CatchFreight("VNo", Vehicle, FromBranch, CurrentDate, ToBranch, FMno);
                                if (Freight == 0)
                                {
                                    //From-Parent => TO
                                    Freight = CatchFreight("VNo", Vehicle, tfatBranchFrom.Code, CurrentDate, ToBranch, FMno);
                                    if (Freight == 0)
                                    {
                                        //From => TO-Parent
                                        Freight = CatchFreight("VNo", Vehicle, FromBranch, CurrentDate, tfatBranchTo.Code, FMno);
                                        if (Freight == 0)
                                        {
                                            ...
                                            if (Freight == 0)
                                            {
                                                ...
                                                {
                                                    //From TO
                                                    Freight = CatchFreight("VCategory", VehicleCategory, FromBranch, CurrentDate, ToBranch, FMno);
                                                    if (Freight == 0)
                                                    {
                                                        //From-Parent => TO
                                                        Freight = CatchFreight("VCategory", VehicleCategory, tfatBranchFrom.Code, CurrentDate, ToBranch, FMno);
                                                        if (Freight == 0)
                                                        {
                                                            //From => TO-Parent
                                                            Freight = CatchFreight("VCategory", VehicleCategory, FromBranch, CurrentDate, tfatBranchTo.Code, FMno);
                                                            if (Freight == 0)
                                                            {
                                                                //From-Parent => TO-Parent
                                                                Freight = CatchFreight("VCategory", VehicleCategory, tfatBranchFrom.Code, CurrentDate, tfatBranchTo.Code, FMno);
                                                                if (FromBranch == ToBranch)
                                                                {
                                                                    decimal LocalFreight = CatchLocalFreight("VCategory", VehicleCategory, FromBranch, CurrentDate, FMno);
                                                                ...
                                                                else
                                                                {
                                                                    decimal LocalFreight = CatchLocalFreight("VCategory", VehicleCategory, FromBranch, CurrentDate, FMno);
                                                                    Freight += LocalFreight;
                                                                    LocalFreight = CatchLocalFreight("VCategory", VehicleCategory, ToBranch, CurrentDate, FMno);
                                                                    Freight += LocalFreight;
                                                                }
                                                            }
                                                            else
                                                            {
                                                                decimal LocalFreight = CatchLocalFreight("VCategory", VehicleCategory, ToBranch, CurrentDate, FMno);
                                                                Freight += LocalFreight;
                                                            }
                                                        }
                                                        else
                                                ...
                                                else
                                                {
                                                    if (FromBranch == ToBranch)
                                                    {
                                                        decimal LocalFreight = CatchLocalFreight("VNo", Vehicle, FromBranch, CurrentDate, FMno);
                                                        Freight += LocalFreight;
                                                    }
                                                    else
                                                    {
                                                        decimal LocalFreight = CatchLocalFreight("VNo", Vehicle, FromBranch, CurrentDate, FMno);
                                                        Freight += LocalFreight;
                                                        LocalFreight = CatchLocalFreight("VNo", Vehicle, ToBranch, CurrentDate, FMno);
                                                        Freight += LocalFreight;
                                                    }
                                                }
                                            }
                                        ...
                                        {
                                            decimal LocalFreight = CatchLocalFreight("VNo", Vehicle, ToBranch, CurrentDate, FMno);
                                            Freight += LocalFreight;
                                        }
                                    }
                                    else
                            ...
                            {
                                //From TO
                                Advance = CatchAdvance("VNo", Vehicle, FromBranch, CurrentDate, ToBranch, FMno);
                                if (Advance == 0)
                                {
                                    //From-Parent => TO
                                    Advance = CatchAdvance("VNo", Vehicle, tfatBranchFrom.Code, CurrentDate, ToBranch, FMno);
                                    if (Advance == 0)
                                    {
                                        //From => TO-Parent
                                        Advance = CatchAdvance("VNo", Vehicle, FromBranch, CurrentDate, tfatBranchTo.Code, FMno);
                                        if (Advance == 0)
                                        {
                                            //From-Parent => TO-Parent
                                            Advance = CatchAdvance("VNo", Vehicle, FromBranch, CurrentDate, tfatBranchTo.Code, FMno);
                                            if (Advance == 0)
                                            {
                                                ...
                                                {
                                                    //From TO
                                                    Advance = CatchAdvance("VCategory", VehicleCategory, FromBranch, CurrentDate, ToBranch, FMno);
                                                    if (Advance == 0)
                                                    {
                                                        //From-Parent => TO
                                                        Advance = CatchAdvance("VCategory", VehicleCategory, tfatBranchFrom.Code, CurrentDate, ToBranch, FMno);
                                                        if (Advance == 0)
                                                        {
                                                            //From => TO-Parent
                                                            Advance = CatchAdvance("VCategory", VehicleCategory, FromBranch, CurrentDate, tfatBranchTo.Code, FMno);
                                                            if (Advance == 0)
                                                            {
                                                                //From-Parent => TO-Parent
                                                                Advance = CatchAdvance("VCategory", VehicleCategory, tfatBranchFrom.Code, CurrentDate, tfatBranchTo.Code, FMno);
                                                                if (FromBranch == ToBranch)
                                                                {
                                                                    decimal LocalFreight = CatchLocalAdvance("VCategory", VehicleCategory, FromBranch, CurrentDate, FMno);
                                                                    Advance += LocalFreight;
                                                                }
                                                                else
                                                                {
                                                                    decimal LocalFreight = CatchLocalAdvance("VCategory", VehicleCategory, FromBranch, CurrentDate, FMno);
                                                                    Advance += LocalFreight;
                                                                    LocalFreight = CatchLocalAdvance("VCategory", VehicleCategory, ToBranch, CurrentDate, FMno);
                                                                    Advance += LocalFreight;
                                                                }
                                                            }
                                                            else
                                                            {
                                                                decimal LocalFreight = CatchLocalAdvance("VCategory", VehicleCategory, ToBranch, CurrentDate, FMno);
                                                                Advance += LocalFreight;
                                                            }
                                                        }
                                                        else
                                                ...
                                                else
                                                {
                                                    if (FromBranch == ToBranch)
                                                    {
                                                        decimal LocalFreight = CatchLocalAdvance("VNo", Vehicle, FromBranch, CurrentDate, FMno);
                                                        Advance += LocalFreight;
                                                    }
                                                    else
                                                    {
                                                        decimal LocalFreight = CatchLocalAdvance("VNo", Vehicle, FromBranch, CurrentDate, FMno);
                                                        Advance += LocalFreight;
                                                        LocalFreight = CatchLocalAdvance("VNo", Vehicle, ToBranch, CurrentDate, FMno);
                                                        Advance += LocalFreight;
                                                    }
                                                }
                                            }
                                        ...
                                        {
                                            decimal LocalFreight = CatchLocalAdvance("VNo", Vehicle, ToBranch, CurrentDate, FMno);
                                            Advance += LocalFreight;
                                        }
                                    }
                                    else
                            ...
                            #region Get ViaAdvance && ViaFreight
                            ...
                            if (!String.IsNullOrEmpty(RouteVia))
                            {
                                ...
                                {
                                    ViaFreight = CatchViaFreight("VNo", Vehicle, item, CurrentDate, FMno);
                                    if (ViaFreight == 0)
                                    {
                                        ViaFreight = CatchViaFreight("VCategory", VehicleCategory, item, CurrentDate, FMno);
                                    }
                                    Freight += ViaFreight;
                                    ViaAdvance = CatchViaAdvance("VNo", Vehicle, item, CurrentDate, FMno);
                                    if (ViaAdvance == 0)
                                    {
                                        ViaAdvance = CatchViaAdvance("VCategory", VehicleCategory, item, CurrentDate, FMno);
                                    }
                                    Advance += ViaAdvance;
                        ...
                        else if (vehicleMaster.PostReq == true && vehicleMaster.MaintainDriverAC == false)
                        {
                            //Catch Freight And Advance Both At A Time
                            var RateType = vehicleMaster.RateType.Split(',').ToList();
                            #region Freight
                ...
                else
                {
                    ...
                    {
                        if (hireVehicleMaster.PickVehicleRate)
                        {
                            DateTime CurrentDate = ConvertDDMMYYTOYYMMDD(FMDate);
                            TfatBranch tfatBranchFrom = new TfatBranch();
                            TfatBranch tfatBranchTo = new TfatBranch();
                            bool CheckParent = hireVehicleMaster.GetParentRateAlso;
                            if (CheckParent)
                            {
                                tfatBranchFrom = ctxTFAT.TfatBranch.Where(x => x.Code == FromBranch).FirstOrDefault();
                                if (tfatBranchFrom.Category == "Area" || tfatBranchFrom.Category == "SubBranch")
        ...
        public ActionResult ShowCharges(bool Freight)
        {
            ...
            if (Freight)
            {
                ...
                FMCharges1.AddRange(FMCharges.Where(x => x.Type == "F" || x.Type == "LF" || x.Type == "VF" || x.Type == "FA" || x.Type == "LFA" || x.Type == "VFA").ToList());
                ...
                FMCharges1.ForEach(x =>
                {
                    ...
                    x.AUTHIDS = x.Type == "F" ? "Freight" : x.Type == "LF" ? "Local Freight" : x.Type == "VF" ? "Via Freight" : x.Type == "FA" ? "Freight" : x.Type == "LFA" ? "Local Freight" : "Via Freight";
            ...
            else
            {
                ...
                FMCharges1.AddRange(FMCharges.Where(x => x.Type == "A" || x.Type == "LA" || x.Type == "VA" || x.Type == "FA" || x.Type == "LFA" || x.Type == "VFA").ToList());
                //Session["FMFreightAdvance"] = FMCharges;
                FMCharges1.ForEach(x =>
                {
                    x.Amt = x.Amt1 ?? 0;
                    x.AUTHORISE = ctxTFAT.TfatBranch.Where(c => c.Code == x.FromBranch).Select(c => c.Name).FirstOrDefault();
                    x.ENTEREDBY = ctxTFAT.TfatBranch.Where(c => c.Code == x.ToBranch).Select(c => c.Name).FirstOrDefault();
                    x.AUTHIDS = x.Type == "A" ? "Advance" : x.Type == "LA" ? "Local Advance" : x.Type == "VA" ? "Via Advance" : x.Type == "FA" ? "Advance" : x.Type == "LFA" ? "Local Advance" : "Via Advance";
                });
            }
        ...
        public ActionResult GetPostingNew(FMVM Model)
        {
            ...
            //bool mMaintainCredit = Model.VehicleGroup == "100001" ? false : vehicleMaster.ARAP;


            //if (Model.VehicleGroup != "100001" && vehicleMaster.PostReq == false && mMaintainDriverac == false)
            //{
            //    return Json(new
            //    {
            //        Status = "CancelError",
            //        Message = "Cant Save The Entry"
            //    }, JsonRequestBehavior.AllowGet);
            //}
        ...
        public string GetValidationPosting(FMVM mModel)
        {
            ...
            else
            {
                ...
                string mDriverPostCode = mModel.VehicleGroup == "100001" ? null : ctxTFAT.DriverMaster.Where(x => x.Code == mvehicleMaster.Driver).Select(x => x.Posting).FirstOrDefault();
        ...
        public ActionResult SaveData(FMVM mModel)
        {
            ...
            using (var transaction = ctxTFAT.Database.BeginTransaction())
            {
                try
                {
                    ...
                    //{
                    //    if (!mModel.Document.Contains('D'))
                    //    {
                    //        FMNO = Convert.ToInt32(mModel.Document);
                    //    }
                    //}
                    ...
                    TfatUserAuditHeader authorisation = ctxTFAT.TfatUserAuditHeader.Where(x => x.Type == "FMH00").FirstOrDefault();
                    ...
                    #region Darshan
                    //List<PurchaseVM> LedgerPosting = GetPostingNew(mModel);
                    ...
                    if (Posting)
                    {
                        ...
                        if (ledpost != null)
                        {
                            for (int u = 0; u < ledpost.Count; u++)
                            {
                                ...
                                mobjL.TableKey = mbranchcode + fMMaster.Type + mperiod.Substring(0, 2) + lCnt.ToString("D3") + fMMaster.FmNo;
                                mobjL.PCCode = 100002;
                        ...
                        if (mMaintainDriverac == true)
                        {
                            mDriverAc = vehicleMaster.Driver;
                            mDriverPostCode = ctxTFAT.DriverMaster.Where(x => x.Code == mDriverAc).Select(x => x.Posting).FirstOrDefault();
                        }
                    ...
                    if (Posting)
                    {
                        if (mModel.TDSAmt != 0)
                        {
                            ...
                            tspay.SubType = "LC";
                            tspay.TableKey = mbranchcode + fMMaster.Type + mperiod.Substring(0, 2) + 1.ToString("D3") + fMMaster.FmNo;
                    ...
                    transaction.Commit();
                    transaction.Dispose();
                    SendSMS_MSG_Email(mModel.Mode, fMMaster.Freight, fMMaster.Branch + fMMaster.ParentKey, fMMaster.Date, fMMaster.BroCode, "A","FMH00");
                    UpdateAuditTrail(mbranchcode, mModel.Mode, mModel.Header, fMMaster.ParentKey, fMMaster.Date, fMMaster.Freight, fMMaster.BroCode, "Save Hire-Freight Memo :" + fMMaster.FmNo, "A");
        ...
        public ActionResult UpDateFMRouteData(FMVM mModel)
        {
            ...
            using (var transaction = ctxTFAT.Database.BeginTransaction())
            {
                try
                {
                    ...
                    transaction.Commit();
                    transaction.Dispose();
                    UpdateAuditTrail(mbranchcode, mModel.Mode, mModel.Header, fMMaster.ParentKey, DateTime.Now, fMMaster.Freight, fMMaster.BroCode, "Save Hire Freight Memo", "A");
                }
        ...
        public string DeleteStateMaster(FMVM mModel)
        {
            ...
            var GetRemarkDocList = ctxTFAT.Narration.Where(x => x.Srl == fMMaster.FmNo.ToString() ).ToList();
            ...
            var FMAttachMent = ctxTFAT.Attachment.Where(x =>  x.Srl == fMMaster.FmNo.ToString()).ToList();
            ...
            ctxTFAT.SaveChanges();
            UpdateAuditTrail(mbranchcode, mModel.Mode, mModel.Header, fMMaster.ParentKey, fMMaster.Date, fMMaster.Freight, fMMaster.BroCode, "Delete Hire-Freight Memo :" + fMMaster.FmNo, "A");
        ...
        public ActionResult SaveDraftData(FMVM mModel)
        {
            using (var transaction = ctxTFAT.Database.BeginTransaction())
            {
                try
                {
                    ...
                    transaction.Commit();
                    transaction.Dispose();
                    UpdateAuditTrail(mbranchcode, mModel.Mode, mModel.Header, "", DateTime.Now, fMMaster.Freight.Value, fMMaster.BroCode, "Save Draft", "A");
                }
