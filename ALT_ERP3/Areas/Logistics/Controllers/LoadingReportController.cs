...
{
    public class LoadingReportController : BaseController
    {
        ...
        public ActionResult Index(LoadingUnloagingVM Model)
        {
            ...
            tfatSearch = ctxTFAT.TfatSearch.Where(x => x.Code == "DailySummaryLoading").ToList();
            tfatSearch.ForEach(x => x.IsHidden = false);
        ...
        public ActionResult GetGridStructureRecords(LoadingUnloagingVM Model)
        {
            try
            {
                TempData["LVehicle"] = Model.Vehicle;
                TempData["LBranch"] = Model.Branch;
                TempData["LUnit"] = Model.Unit;
        ...
        {
            Model.Vehicle = TempData.Peek("LVehicle") as string;
            Model.Branch = TempData.Peek("LBranch") as string;
            Model.Unit = TempData.Peek("LUnit") as string;

            if (String.IsNullOrEmpty(Model.mWhat))
            {
                if (Model.ViewDataId.Trim() == "LoadingReport")
                {
                    ExecuteStoredProc("Drop Table ztmp_LoadingDetails");
                    SqlConnection tfat_conx = new SqlConnection(GetConnectionString());
                    SqlCommand cmd = new SqlCommand("SPTFAT_LoadingReport", tfat_conx);
