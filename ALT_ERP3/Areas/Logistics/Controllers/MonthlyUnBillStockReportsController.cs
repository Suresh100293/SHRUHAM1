...
{
    public class MonthlyUnBillStockReportsController : BaseController
    {
        ...
        // GET: Logistics/MonthlyUnBillStockReports
        public ActionResult Index(MonthlyBookingUnBillLRVM Model)
        {
            ...
            var DeliveryReporttfatSearch = ctxTFAT.TfatSearch.Where(x => x.Code == "MonthlyStockReportsAgeReports").ToList();
        ...
        public ActionResult GetGridStructureRecords(MonthlyBookingUnBillLRVM Model)
        {
            try
            {
                ...
                if (Model.ViewDataId == "MonthlyStockReports")
                {
                    ExecuteStoredProc("Drop Table ztmpmothlyUnbillLRStock_zOS");
                    SqlConnection tfat_conx = new SqlConnection(GetConnectionString());
                    SqlCommand cmd = new SqlCommand("SPREP_MonthlyStockReports", tfat_conx);
                    ...
                    cmd.Parameters.Add("@mStockType", SqlDbType.VarChar).Value = Model.StockType == null ? "" : Model.StockType;
        ...
        public ActionResult GetGridData(MonthlyBookingUnBillLRVM Model)
        {
            ...
            var ReportType = "X";
            if (Model.ViewDataId == "MonthlyStockReportsAgeReports")
            {
                ReportType = "R";
            }
        ...
        public ActionResult GetParameterAuto(MonthlyBookingUnBillLRVM Model)
        {
            ...
            if (ctxTFAT.ReportParameters.Where(x => x.Reports.Trim().ToLower() == MainReportName.Trim().ToLower() && x.ReportName.Trim().ToLower() == Model.ReportTypeL.Trim().ToLower() && x.ReportNameAlias.Trim().ToLower() == Model.ReportName.Trim().ToLower()).FirstOrDefault() != null)
            {
                ...
                Model.StockType = mobj.Para6 == null ? "" : mobj.Para6;
            ...
            if (Model.TotalAmt)
            {
                DeliveryReporttfatSearch.Where(x => x.ColHead.ToUpper().Contains("VAL")).ToList().ForEach(x => x.IsHidden = true);
            }
            if (Model.TotalQty)
            {
        ...
        public ActionResult SaveParameter(MonthlyBookingUnBillLRVM Model)
        {
            ...
            using (var transaction = ctxTFAT.Database.BeginTransaction())
            {
                try
                {
                    ...
                    mobj.Para6 = Model.StockType == null ? "" : Model.StockType;
        ...
        public void NoOfLRDetailsColumns(bool Flag, string ViewDataId)
        {
            ...
            var DtfatSearch = ctxTFAT.TfatSearch.Where(x => x.Code == "MonthlyStockReportsAgeReports").ToList();
            if (!Flag)
            {
        ...
        public void TotalAmtDetailsColumns(bool Flag, string ViewDataId)
        {
            ...
            if (!Flag)
            {
                tfatSearch.Where(x => x.ColHead.ToUpper().Contains("VAL")).ToList().ForEach(x => x.IsHidden = true);
                DtfatSearch.Where(x => x.ColHead.ToUpper().Contains("VAL")).ToList().ForEach(x => x.IsHidden = true);
            }
            else
            {
                tfatSearch.Where(x => x.ColHead.ToUpper().Contains("VAL")).ToList().ForEach(x => x.IsHidden = false);
                DtfatSearch.Where(x => x.ColHead.ToUpper().Contains("VAL")).ToList().ForEach(x => x.IsHidden = false);
            }
            ctxTFAT.SaveChanges();
        }
        #endregion
