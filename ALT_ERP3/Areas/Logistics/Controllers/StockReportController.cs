...
namespace ALT_ERP3.Areas.Logistics.Controllers
{
    public class StockReportController : BaseController
    {
        ...
        private List<SelectListItem> PopulateStockBranchesOnly()
        {
            ...
            using (SqlConnection con = new SqlConnection(GetConnectionString()))
            {
                string query = "SELECT Code, Name FROM TfatBranch where Category<>'Area' and  Category<>'Zone'  order by Recordkey ";
                using (SqlCommand cmd = new SqlCommand(query))
                {
                    cmd.Connection = con;
                    con.Open();
        ...
        private List<SelectListItem> PopulateAreaOnly()
        {
            List<SelectListItem> items = new List<SelectListItem>();
            using (SqlConnection con = new SqlConnection(GetConnectionString()))
            {
        ...
        private List<SelectListItem> PopulateStockType()
        {
            List<SelectListItem> items = new List<SelectListItem>();
            items.Add(new SelectListItem
            {
        ...
        // GET: Logistics/StockReport
        public ActionResult Index(StockReportVM Model)
        {
            ...
            Model.StockBranchList = PopulateStockBranchesOnly();
            Model.StockAreaList = PopulateAreaOnly();
            Model.VehicleList = PopulateVehicle();
            Model.StockTypes = PopulateStockType();
        ...
        [HttpPost]
        public ActionResult GetGridStructureRecords(StockReportVM Model)
        {
            try
            {
                TempData["StockBranch"] = Model.Branch;
                TempData["StockStockBranch"] = Model.StockBranch;
                TempData["StockVehicle"] = Model.Vehicle;
                TempData["StockStockType"] = Model.StockType;
                TempData["StockStockArea"] = Model.StockArea;
        ...
        public ActionResult GetGridData(StockReportVM Model)
        {
            if (String.IsNullOrEmpty(Model.mWhat))
            {
                if (Model.ViewDataId.Trim().ToLower() == "StockReport".ToLower())
                {
                    ExecuteStoredProc("Drop Table ztmp_StockReport");
                    SqlConnection tfat_conx = new SqlConnection(GetConnectionString());
                    SqlCommand cmd = new SqlCommand("StockReport", tfat_conx);
                    ...
                    cmd.Parameters.Add("@mBranch", SqlDbType.VarChar).Value = Convert.ToString(TempData.Peek("StockBranch"));
                    cmd.Parameters.Add("@mStockBranch", SqlDbType.VarChar).Value = Convert.ToString(TempData.Peek("StockStockBranch"));
                    cmd.Parameters.Add("@mStockArea", SqlDbType.VarChar).Value = Convert.ToString(TempData.Peek("StockStockArea"));
                    cmd.Parameters.Add("@mVehicle", SqlDbType.VarChar).Value = Convert.ToString(TempData.Peek("StockVehicle"));
                    cmd.Parameters.Add("@mStockType", SqlDbType.VarChar).Value = Convert.ToString(TempData.Peek("StockStockType"));
        ...
        [HttpPost]
        public ActionResult ParameterReset(StockReportVM Model)
        {
            Model.StockAreaList = PopulateAreaOnly();
            Model.BranchList = PopulateBranchesOnly();
            Model.StockBranchList = PopulateStockBranchesOnly();
        ...
        [HttpPost]
        public ActionResult GetParameterAuto(StockReportVM Model)
        {
        ...
        [HttpPost]
        public ActionResult SaveParameter(StockReportVM Model)
        {
            var MainReportName = "CashMemoReport";
        ...
        public ActionResult DeleteParamete(StockReportVM Model)
        {
