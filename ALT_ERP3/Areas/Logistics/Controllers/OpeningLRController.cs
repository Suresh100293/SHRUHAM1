...
{
    public class OpeningLRController : BaseController
    {
        ...
        public ActionResult CheckLRDate(string DocDate, string DocTime)
        {
            ...
            if (ConvertDDMMYYTOYYMMDD(DocDate) <= prevfirstdate)
            {
                Status = "T";
            }
            else
            {
                Status = "F";
                message = "LRDATE Should Be Less Than " + prevfirstdate.ToShortDateString() + " ...!";
            }
        ...
        public ActionResult GetCustomerDetails(string BillingParty)
        {
            ...
            string Coln = "", Deli = "", HitContract = "C";
            if (!(string.IsNullOrEmpty(BillingParty)))
            {
            ...
            return Json(new { HitContract = HitContract, POnumver = POnumver, BEnumber = BEnumber, PartyChallan = PartyChallan, PartyInvoice = PartyInvoice, Coln = Coln, Deli = Deli, JsonRequestBehavior.AllowGet });
        }
        ...
        public ActionResult CheckManualLR(int lrno, string document)
        {
            ...
            {
                LRMaster lRMaster = ctxTFAT.LRMaster.Where(x => x.LrNo == lrno && x.TableKey.ToString() != document).FirstOrDefault();
                if (lRMaster != null)
                {
                    Flag = "T";
        ...
        public ActionResult Index(LRVM mModel)
        {
            ...
            UpdateAuditTrail(mbranchcode, mModel.Mode, mModel.Header, mModel.Document, DateTime.Now, 0, "", "", "NA");
            ...
            if ((mModel.Mode == "Edit") || (mModel.Mode == "View") || (mModel.Mode == "Delete") || (mModel.getRecentLR == true))
            {
                ...
                else
                {
                    ...
                    var Stock = ctxTFAT.LRStock.Where(x => x.ParentKey == mlist.TableKey).FirstOrDefault();
                    if (Stock != null)
                    {
                        mModel.Branch = Stock.Branch;
                        mModel.Branch_Name = ctxTFAT.TfatBranch.Where(x => x.Code == mModel.Branch).Select(x => x.Name).FirstOrDefault();
                        mModel.StockQty = Stock.TotalQty;
                    }
                ...
                if (mlist != null)
                {
                    ...
                    foreach (var i in trncharges)
                    {
                        ...
                        c.Val1 = GetChargeValValue(c.tempId, "Val", mlist.LrNo);
        ...
        public decimal GetChargeValValue(int i, string mfield, int LRNO)
        {
            string connstring = GetConnectionString();
            string abc;
            var loginQuery3 = @"select " + mfield + i + " from LRMaster where LRno=" + LRNO + " ";
            ...
            using (var transaction = ctxTFAT.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (ctxTFAT.LRMaster.Where(x => (x.TableKey.ToString() == mModel.Document)).FirstOrDefault() != null)
                    {
                        ...
                        if (ctxTFAT.LRStock.Where(x => (x.ParentKey.ToString() == mModel.Document)).FirstOrDefault()!=null)
                        {
                    ...
                    var doct = ConvertDDMMYYTOYYMMDD(Model.BookDate);
                    var Prefix = mperiod;
                    if (mAdd)
                    {
                        ...
                        lRMaster.ParentKey = mbranchcode + "LR000" + Prefix.Substring(0, 2) + lRMaster.LrNo;
                        lRMaster.TableKey = mbranchcode + "LR000" + Prefix.Substring(0, 2) + 1.ToString("D3") + lRMaster.LrNo;
                    ...
                    if (StkmAdd)
                    {
                        ...
                        lRStock.TableKey = mbranchcode + "STK00" + Prefix.Substring(0, 2) + 1.ToString("D3") + lRMaster.LrNo;
                    }
                    //PickOrder
                    ...
                    //LRMaster
                    {
                        ...
                        if (mModel.LRDetailList != null)
                        {
                            foreach (var item in mModel.LRDetailList)
                            {
                                ...
                                lrRelatedExp.Tablekey = mbranchcode + "LR000" + Prefix.Substring(0, 2) + item.tempId.ToString("D3") + lRMaster.LrNo;
                    ...
                    //LRStock
                    {
                        ...
                        lRStock.TotalQty = mModel.StockQty;
                        lRStock.BalQty = mModel.StockQty;
                        lRStock.AllocatBalQty = mModel.StockQty;
                    ...
                    if (mModel.StockQty > 0)
                    {
                        if (StkmAdd)
                        {
                            lRStock.Prefix = Prefix;
        ...
        {
            int Status = 0;
            TfatMenu tfatMenu = ctxTFAT.TfatMenu.Where(x => x.FormatCode.Trim().ToUpper() == mModel.OptionCode.Trim().ToUpper()).FirstOrDefault();
            if (tfatMenu != null)
            {
        ...
        //public ActionResult SaveData(OpeningLRVM mModel)
        //{

        //    OpeningLRVM Model = mModel;
        //    using (var transaction = ctxTFAT.Database.BeginTransaction())
        //    {
        //        try
        //        {
        ...
        //                if (ctxTFAT.LRStock.Where(x => x.ParentKey == lRMaster.TableKey).FirstOrDefault()!=null)
        //                {
        //                    lRStock = ctxTFAT.LRStock.Where(x => (x.LrNo.ToString() == mModel.Document)).FirstOrDefault();
        //                    stkmAdd = false;
        //                }
        //                mAdd = false;
        //            }
        ...
        //            DateTime firstdate = ctxTFAT.TfatPerd.OrderBy(z => z.StartDate).Select(x => x.StartDate).FirstOrDefault();
        //            var prevfirstdate = firstdate.AddDays(-1);
        //            //var Dates = ConvertDDMMYYTOYYMMDD(Model.BookDate);
        //            //var Prefix = GetPrefix(Dates);
        //            var doct = ConvertDDMMYYTOYYMMDD(Model.BookDate);
        ...
        //                lRMaster.POD = true;
        //                lRMaster.Crossing = mModel.Crossing;
        //                lRMaster.ParentKey = "LR000" + Prefix.Substring(0, 2) + mModel.LrNo;
        //                lRMaster.TableKey = "LR000" + Prefix.Substring(0, 2) + 1.ToString("D3") + mModel.LrNo;
        //                lRMaster.BENumber = mModel.BENumber;
        //                lRMaster.PONumber = mModel.PONumber;
        ...
        //                {
        //                    lRStock.ParentKey = lRMaster.TableKey;
        //                    lRStock.TableKey = "STK00" + Prefix.Substring(0, 2) + 1.ToString("D3") + mModel.LrNo;
        //                    lRStock.Branch = mModel.Branch;
        //                    lRStock.Date = ConvertDDMMYYTOYYMMDD(mModel.BookDate);
        //                    lRStock.Time = mModel.Time;
        //                    lRStock.Type = "LR";
        ...
        //            if (authorisation != null)
        //            {
        //                var Athorise = SetAuthorisationLogistics(authorisation, lRMaster.TableKey, lRMaster.LrNo.ToString(), 0, lRMaster.BookDate.ToShortDateString(), lRMaster.Val1, lRMaster.BillParty, mbranchcode);
        //                lRStock.AUTHORISE = Athorise;
        //                lRMaster.AUTHORISE = Athorise;
        //            }
        //            #endregion
        ...
        //                ctxTFAT.LRMaster.Add(lRMaster);
        //                AttachmentVM vM = new AttachmentVM();
        //                vM.ParentKey = lRMaster.ParentKey;
        //                vM.Srl = lRMaster.LrNo.ToString();
        //                vM.Prefix = Prefix;
        //                SaveAttachment(vM);
        //            }
        ...
        //            UpdateAuditTrail(mbranchcode, mModel.Mode, mModel.Header, lRMaster.ParentKey, DateTime.Now, 0, lRMaster.BillParty, "Saved Opening Consignment", "CA");
        //        }
        //        catch (DbEntityValidationException ex1)
        //        {
        //            transaction.Rollback();
        //            return Json(new { Message = ex1.InnerException.Message, Status = "Error", id = "OpeningLR" }, JsonRequestBehavior.AllowGet);
        //        }
        //        catch (System.Data.Entity.Infrastructure.DbUpdateException ex)
        //        {
        //            transaction.Rollback();
        //            return Json(new { Message = ex.InnerException.Message, Status = "Error", id = "OpeningLR" }, JsonRequestBehavior.AllowGet);
        //        }
        //        catch (Exception e)
        //        {
        //            ModelState.AddModelError("", "Unable to Save Changes. Try again, and if the problem persists, Contact your System Administrator.");
        ...
        //    return Json(new { Status = "Success", id = "OpeningLR" }, JsonRequestBehavior.AllowGet);
        //}
