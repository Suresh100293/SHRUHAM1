...
{
    public class EwayLogActivityController : BaseController
    {
        ...
        private List<SelectListItem> PopulateDocTypesOnly()
        {
            ...
            using (SqlConnection con = new SqlConnection(GetConnectionString()))
            {
                string query = "select distinct Description from EWBLog ";
                using (SqlCommand cmd = new SqlCommand(query))
                {
                    cmd.Connection = con;
                    con.Open();
                    using (SqlDataReader sdr = cmd.ExecuteReader())
                    {
                        while (sdr.Read())
                        {
                            items.Add(new SelectListItem
                            {
                                ...
                                Value = sdr["Description"].ToString()
                            });
                        }
                    }
                    con.Close();
        ...
        // GET: Logistics/EwayLogActivity
        public ActionResult Index(EWBLogVM Model)
        {
            ...
            Model.Descriptions = PopulateDocTypesOnly();
        ...
        [HttpPost]
        public ActionResult GetGridStructureRecords(EWBLogVM Model)
        {
            try
            {
                TempData["EWBLogBranch"] = Model.Branch;
                TempData["EWBLogUser"] = Model.User;
                TempData["EWBLogDescription"] = Model.Description;
        ...
        public ActionResult GetGridData(EWBLogVM Model)
        {
            if (String.IsNullOrEmpty(Model.mWhat))
            {
                ExecuteStoredProc("Drop Table ztmp_EWBLogReports");
                SqlConnection tfat_conx = new SqlConnection(GetConnectionString());
                SqlCommand cmd = new SqlCommand("SP_EWBLogReports", tfat_conx);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandTimeout = 120;

                cmd.Parameters.Add("@mUser", SqlDbType.VarChar).Value = Convert.ToString(TempData.Peek("EWBLogUser"));
                cmd.Parameters.Add("@mBranch", SqlDbType.VarChar).Value = Convert.ToString(TempData.Peek("EWBLogBranch"));
                cmd.Parameters.Add("@mDescription", SqlDbType.VarChar).Value = Convert.ToString(TempData.Peek("EWBLogDescription"));
        ...
        [HttpPost]
        public ActionResult ParameterReset(EWBLogVM Model)
        {
