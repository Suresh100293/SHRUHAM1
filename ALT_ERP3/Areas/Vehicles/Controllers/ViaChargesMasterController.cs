...
{
    public class ViaChargesMasterController : BaseController
    {
        ...
        {
            var mobj = ctxTFAT.ViaChargesMa.OrderByDescending(x => x.RECORDKEY).Select(x => x.DocNo).FirstOrDefault();
        ...
        // GET: Vehicles/ViaChargesMaster
        public ActionResult Index(ViaChargesMaVM Model)
        {
            ...
            {
                var mList = ctxTFAT.ViaChargesMa.Where(x => x.DocNo == Model.Document).FirstOrDefault();
                if (mList != null)
                {
                    ...
                    var List = ctxTFAT.ViaChargesMaRef.Where(x => x.DocNo == Model.Document).ToList();
                    ...
                    var Mobj1 = (from TfatBranch in Mobj
                                 select new ViaChargesMaVM
                                 {
                                     ...
                                     VaiCharge = List.Where(x => x.Area == TfatBranch.ID).Select(x => x.ViaCharges).FirstOrDefault() ?? 0,
                    ...
                    List<ViaChargesMaVM> tripCharges = new List<ViaChargesMaVM>();
        ...
        public ActionResult Via_LocalExp(ViaChargesMaVM Model)
        {
            Session["ViaCharges"] = null;
            List<ViaChargesMaVM> tripCharges = new List<ViaChargesMaVM>();
            ...
            if (Model.copy == false)
            {
                var CheckExist = ctxTFAT.ViaChargesMa.Where(x => x.DocNo != Model.Document && x.VehicleType == Model.VehicleType && x.Vehicle == Model.Vehicle && x.FromPeriod == Statdate ).ToList();
            ...
            {
                bool NewTrip = true;
                var GetTripMaster = ctxTFAT.ViaChargesMa.Where(x => x.DocNo == Model.Document).FirstOrDefault();
                ...
                if (Model.copy)
                {
                    ...
                    var List = ctxTFAT.ViaChargesMaRef.Where(x => x.DocNo == Model.copyDocument && list.Contains(x.Area)).ToList();
                    var Mobj = (from TfatBranch in ctxTFAT.TfatBranch
            ...
            Model.list = tripCharges;
            Session["ViaCharges"] = tripCharges;
            var html = ViewHelper.RenderPartialView(this, "ViaChargesPartial", Model);
            return Json(new { Status = Status, Message = Message, Html = html }, JsonRequestBehavior.AllowGet);
        }

        public ActionResult SaveData(ViaChargesMaVM mModel)
        {
            using (var transaction = ctxTFAT.Database.BeginTransaction())
            {
                try
                {
                    ...
                    ViaChargesMa mobj = new ViaChargesMa();
                    var Statdate = ConvertDDMMYYTOYYMMDD(mModel.FromDate);
                    var CheckExist = ctxTFAT.ViaChargesMa.Where(x => x.DocNo != mModel.Document && x.VehicleType == mModel.VehicleType && x.Vehicle == mModel.Vehicle && x.FromPeriod == Statdate).ToList();
                    ...
                    bool mAdd = true;
                    if (ctxTFAT.ViaChargesMa.Where(x => x.DocNo == mModel.Document).FirstOrDefault() != null)
                    {
                        mobj = ctxTFAT.ViaChargesMa.Where(x => x.DocNo == mModel.Document).FirstOrDefault();
                        mAdd = false;
                        DeUpdate(mModel);
                    }
                    if (mAdd)
                    ...
                    foreach (var item in mModel.list)
                    {
                        ...
                        if (item.VaiCharge != 0 || item.ViaAdvance != 0)
                        {
                            ViaChargesMaRef advRef = new ViaChargesMaRef();
                            advRef.DocNo = mobj.DocNo;
                            advRef.Area = item.ID;
                            //////advRef.LocalFreight = item.ExraFreight;
                            advRef.ViaCharges = item.VaiCharge;
                            ...
                            ctxTFAT.ViaChargesMaRef.Add(advRef);
                        }
                    }
                    if (mAdd == true)
                    {
                        ctxTFAT.ViaChargesMa.Add(mobj);
                    }
                    else
                    {
                    ...
                    transaction.Commit();
                    transaction.Dispose();
                    UpdateAuditTrail(mbranchcode, mModel.Mode, mModel.Header, "     " + mperiod.Substring(0, 2) + mobj.DocNo, DateTime.Now, 0, mobj.DocNo, "Save Driver Via Charges Master", "NA");
                }
        ...
        public void DeUpdate(ViaChargesMaVM Model)
        {
            var BillSubRef = ctxTFAT.ViaChargesMaRef.Where(x => x.DocNo == Model.Document).ToList();
            ctxTFAT.ViaChargesMaRef.RemoveRange(BillSubRef);
            ctxTFAT.SaveChanges();
        }

        public ActionResult DeleteTripMaster(ViaChargesMaVM mModel)
        {
            ...
            var mList = ctxTFAT.ViaChargesMa.Where(x => (x.DocNo == mModel.Document)).FirstOrDefault();
            ctxTFAT.ViaChargesMa.Remove(mList);
            var BillSubRef = ctxTFAT.ViaChargesMaRef.Where(x => x.DocNo == mModel.Document).ToList();
            ctxTFAT.ViaChargesMaRef.RemoveRange(BillSubRef);
            ctxTFAT.SaveChanges();
            UpdateAuditTrail(mbranchcode, mModel.Mode, mModel.Header, "     " + mperiod.Substring(0, 2) + mList.DocNo, DateTime.Now, 0, mList.DocNo, "Delete Driver Via Charges Master", "NA");
        ...
        {
            var Mobj = (from ViaCharges in ctxTFAT.ViaChargesMa
                        where ViaCharges.DocNo != DocNo 
                        select new CopyViaChargesVM
                        {
            ...
            if (Mobj == null)
            {
                Mobj = new List<CopyViaChargesVM>();
            }
            var html = ViewHelper.RenderPartialView(this, "ListOFViaCharges", Mobj);
