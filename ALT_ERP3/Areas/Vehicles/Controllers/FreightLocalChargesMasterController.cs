...
{
    public class FreightLocalChargesMasterController : BaseController
    {
        ...
        public JsonResult GetBranch(string term, bool Branch)
        {
            ...
            if (Branch)
            {
                list = ctxTFAT.TfatBranch.Where(x => (x.Category == "Branch" || x.Category == "SubBranch") && x.Status == true).OrderBy(x => x.Name).ToList();
            }
            else
            {
                list = ctxTFAT.TfatBranch.Where(x => x.Category != "0" && x.Status == true).OrderBy(x => x.Name).ToList();
        ...
        {
            var mobj = ctxTFAT.FreightLocalChargesMa.OrderByDescending(x => x.RECORDKEY).Select(x => x.DocNo).FirstOrDefault();
        ...
        // GET: Vehicles/FreightLocalChargesMaster
        public ActionResult Index(FreightLocalChargesMaVM Model)
        {
            ...
            {
                var mList = ctxTFAT.FreightLocalChargesMa.Where(x => x.DocNo == Model.Document).FirstOrDefault();
                if (mList != null)
                {
                    ...
                    var List = ctxTFAT.FreightLocalChargesMaRef.Where(x => x.DocNo == Model.Document).ToList();
                    var Mobj = (from TfatBranch in ctxTFAT.TfatBranch
                                ...
                                select new FreightLocalChargesMaVM
                                {
                    ...
                    var Mobj1 = (from TfatBranch in Mobj
                                 select new FreightLocalChargesMaVM
                                 {
                                     ...
                                     Freight = List.Where(x => x.Area == TfatBranch.ID).Select(x => x.Freight).FirstOrDefault() ?? 0,
                                     Advance = List.Where(x => x.Area == TfatBranch.ID).Select(x => x.Advance).FirstOrDefault() ?? 0,
                                     Category = TfatBranch.Category,
                                     ID = TfatBranch.ID,
                                     GRP = TfatBranch.GRP,
                                 }).ToList();
                    ...
                    List<FreightLocalChargesMaVM> tripCharges = new List<FreightLocalChargesMaVM>();
                    ...
                    Model.FreightCharges = tripCharges;
                }
            }
            else
            {
        ...
        public ActionResult TripCharges(FreightLocalChargesMaVM Model)
        {
            Session["FreightCharges"] = null;
            List<FreightLocalChargesMaVM> tripCharges = new List<FreightLocalChargesMaVM>();
            ...
            if (Model.copy == false)
            {
                var CheckExist = ctxTFAT.FreightLocalChargesMa.Where(x => x.DocNo != Model.Document && x.Vehicle == Model.Vehicle && x.FromPeriod == Statdate).ToList();
            ...
            {
                bool NewTrip = true;
                var GetTripMaster = ctxTFAT.FreightLocalChargesMa.Where(x => x.DocNo == Model.Document).FirstOrDefault();
                ...
                if (Model.copy)
                {
                    var list = Model.FreightCharges.Select(x => x.ID).ToList();
                    var List = ctxTFAT.FreightLocalChargesMaRef.Where(x => x.DocNo == Model.copyDocument && list.Contains(x.Area)).ToList();
                    var Mobj = (from TfatBranch in ctxTFAT.TfatBranch
            ...
            Model.FreightCharges = tripCharges;
            Session["FreightCharges"] = tripCharges;
            var html = ViewHelper.RenderPartialView(this, "FreightLocalChargesPartial", Model);
            return Json(new { Status = Status, Message = Message, Html = html }, JsonRequestBehavior.AllowGet);
        }

        public ActionResult SaveData(FreightLocalChargesMaVM mModel)
        {
            using (var transaction = ctxTFAT.Database.BeginTransaction())
            {
                try
                {
                    ...
                    FreightLocalChargesMa mobj = new FreightLocalChargesMa();
                    var Statdate = ConvertDDMMYYTOYYMMDD(mModel.FromDate);
                    var CheckExist = ctxTFAT.FreightLocalChargesMa.Where(x => x.DocNo != mModel.Document && x.Vehicle == mModel.Vehicle && x.FromPeriod == Statdate).ToList();
                    ...
                    bool mAdd = true;
                    if (ctxTFAT.FreightLocalChargesMa.Where(x => x.DocNo == mModel.Document).FirstOrDefault() != null)
                    {
                        mobj = ctxTFAT.FreightLocalChargesMa.Where(x => x.DocNo == mModel.Document).FirstOrDefault();
                        mAdd = false;
                        DeUpdate(mModel);
                    }
                    if (mAdd)
                    ...
                    foreach (var item in mModel.FreightCharges)
                    {

                        if (item.Freight != 0 || item.Advance != 0)
                        {
                            FreightLocalChargesMaRef advRef = new FreightLocalChargesMaRef();
                            advRef.DocNo = mobj.DocNo;
                            advRef.Area = item.ID;
                            advRef.Freight = item.Freight;
                            advRef.Advance = item.Advance;
                            advRef.AUTHIDS = muserid;
                            advRef.AUTHORISE = mauthorise;
                            advRef.ENTEREDBY = muserid;
                            advRef.LASTUPDATEDATE = System.DateTime.Now;
                            ctxTFAT.FreightLocalChargesMaRef.Add(advRef);
                        }
                    }
                    if (mAdd == true)
                    {
                        ctxTFAT.FreightLocalChargesMa.Add(mobj);
                    }
                    else
                    {
                    ...
                    transaction.Commit();
                    transaction.Dispose();
                    UpdateAuditTrail(mbranchcode, mModel.Mode, mModel.Header, "     " + mperiod.Substring(0, 2) + mobj.DocNo, DateTime.Now, 0, mobj.DocNo, "Save Freight Local Charges", "NA");
                }
        ...
        public void DeUpdate(FreightLocalChargesMaVM Model)
        {
            var BillSubRef = ctxTFAT.FreightLocalChargesMaRef.Where(x => x.DocNo == Model.Document).ToList();
            ctxTFAT.FreightLocalChargesMaRef.RemoveRange(BillSubRef);
            ctxTFAT.SaveChanges();
        }

        public ActionResult DeleteTripMaster(FreightLocalChargesMaVM mModel)
        {
            ...
            var mList = ctxTFAT.FreightLocalChargesMa.Where(x => (x.DocNo == mModel.Document)).FirstOrDefault();
            ctxTFAT.FreightLocalChargesMa.Remove(mList);
            var BillSubRef = ctxTFAT.FreightLocalChargesMaRef.Where(x => x.DocNo == mModel.Document).ToList();
            ctxTFAT.FreightLocalChargesMaRef.RemoveRange(BillSubRef);
            ctxTFAT.SaveChanges();
            UpdateAuditTrail(mbranchcode, mModel.Mode, mModel.Header, "     " + mperiod.Substring(0, 2) + mList.DocNo, mList.DocDate, 0, mList.DocNo, "Delete Freight Local Charges", "NA");
        ...
        {
            List<CopyFreightLocalVM> List = new List<CopyFreightLocalVM>();
            var Mobj = (from FreightCharge in ctxTFAT.FreightLocalChargesMa
                        where FreightCharge.DocNo != DocNo 
                        select new CopyFreightLocalVM
                        {
                            DocumentNo = FreightCharge.DocNo,
                            FromDate = FreightCharge.FromPeriod,
                            Vehicle = ctxTFAT.VehicleCategory.Where(x => x.Code == FreightCharge.Vehicle).Select(x => x.VehicleCategory1).FirstOrDefault() ?? FreightCharge.Vehicle,
                            FromBranch = ctxTFAT.TfatBranch.Where(x => x.Code == FreightCharge.Branch).Select(x => x.Name).FirstOrDefault()
                        }).ToList();
            if (Mobj == null)
            {
                Mobj = new List<CopyFreightLocalVM>();
            }
            var html = ViewHelper.RenderPartialView(this, "ListOFLocalFreightCharge", Mobj);
