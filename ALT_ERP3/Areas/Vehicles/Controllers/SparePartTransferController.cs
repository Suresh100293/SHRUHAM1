...
{
    public class SparePartTransferController : BaseController
    {
        ...
        public ActionResult GetMasterGridData(GridOption Model)
        {
            ...
            Model.Code = "1=1";
            var result = (List<SparePartTransferVM>)Session["ItemStockDetails"];
            if (result==null)
            {
        ...
        {
            var LastCode = ctxTFAT.ExtraItemStock.Where(x=>x.Type== "ITEMS").OrderByDescending(x => x.Srl).Select(x => x.Srl).FirstOrDefault();
        ...
        public ActionResult GetAcType(string term)
        {
            ...
            var result = ctxTFAT.VehicleMaster.Where(x => x.Acitve == true && x.Code != "99998" && x.Code != "99999").Select(x => new { x.Code, x.TruckNo }).ToList();
            foreach (var item in result)
            {
                maclist.Add(new SelectListItem { Value = item.Code, Text = item.TruckNo });
            }
        ...
        public JsonResult GetVehicle(string term)
        {
            ...
            {
                Code = x.Code,
                Name = x.TruckNo + (x.TruckStatus == "100000" ? " -A" : " -O")
            });
            ...
            if ((mModel.Mode == "Edit") || (mModel.Mode == "View") || (mModel.Mode == "Delete"))
            {
                ...
                if (mList != null)
                {
                    ...
                    foreach (var item in ItemList)
                    {
                        ...
                        sparePart.VehicleNoN = ctxTFAT.VehicleMaster.Where(x=>x.Code==item.VehicleNo).Select(x=>x.TruckNo).FirstOrDefault();
                    ...
                    Session["ItemStockDetails"] = mModel.ItemList;
                }
            }
        ...
        public ActionResult GetBreakLedger(SparePartTransferVM Model)
        {
            ...
            foreach (var item in result1)
            {
                ...
                Model.VehicleNoN = ctxTFAT.VehicleMaster.Where(x => x.Code == item.VehicleNo).Select(x => x.TruckNo).FirstOrDefault();
        ...
        public ActionResult AddEditSelectedLedger(SparePartTransferVM Model)
        {
            if (Model.Mode == "Add")
            {
                ...
                if (Session["ItemStockDetails"] != null)
                {
                    objledgerdetail = (List<SparePartTransferVM>)Session["ItemStockDetails"];
                }
                ...
                {
                    item.VehicleNoN = ctxTFAT.VehicleMaster.Where(x => x.Code == item.VehicleNo).Select(x => x.TruckNo).FirstOrDefault();
                    item.Srno = I++;
                }
                objledgerdetail.AddRange(NewList);
                Session.Add("ItemStockDetails", objledgerdetail);
            ...
            else
            {
                ...
                Item.VehicleNoN =ctxTFAT.VehicleMaster.Where(x=>x.Code== Model.VehicleNo).Select(x=>x.TruckNo).FirstOrDefault();
        ...
        {
            var result2 = (List<SparePartTransferVM>)Session["ItemStockDetails"];
            ...
            Session.Add("ItemStockDetails", result);
            Model.ItemList = result;
            var html = ViewHelper.RenderPartialView(this, "GridDetails", Model);
            return Json(new { Html = html }, JsonRequestBehavior.AllowGet);
        }
            ...
            using (var transaction = ctxTFAT.Database.BeginTransaction(System.Data.IsolationLevel.Unspecified))
            {
                try
                {
                    ...
                    if (mAdd)
                    {
                        ...
                        tyreStock.Tablekey = "ITEMS" + mperiod.Substring(0, 2) + "001" + tyreStock.Srl;
                        tyreStock.Parentkey = "ITEMS" + mperiod.Substring(0, 2) + tyreStock.Srl;
                        tyreStock.Type = "ITEMS";
                        Model.ParentKey = tyreStock.Tablekey;
                    ...
                    foreach (var item in result2)
                    {
                        ...
                        useItem.Tablekey = "ITEMS" + mperiod.Substring(0, 2) + Xcnt.ToString("D3") + tyreStock.Srl;
                        useItem.Type = "ITEMS";
                    ...
                    UpdateAuditTrail(mbranchcode, Model.Mode, Model.Header, Parentkey, ConvertDDMMYYTOYYMMDD(Model.DocDate), 0, "", "Save SparePart Stock Transfer", "NA");
        ...
        public ActionResult DeleteData(SparePartTransferVM Model)
        {
            using (var transaction = ctxTFAT.Database.BeginTransaction(System.Data.IsolationLevel.Unspecified))
            {
                try
                {
                    ...
                    DeUpdate(Model);
                    UpdateAuditTrail(mbranchcode, "Delete", Model.Header, mlist.Parentkey, mlist.DocDate, 0, "", "Delete SparePart Stock Transfer", "NA");
