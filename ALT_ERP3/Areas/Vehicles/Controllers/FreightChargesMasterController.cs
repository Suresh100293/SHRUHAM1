...
{
    public class FreightChargesMasterController : BaseController
    {
        ...
        {
            var mobj = ctxTFAT.FreightChargeMa.OrderByDescending(x => x.RECORDKEY).Select(x => x.DocNo).FirstOrDefault();
        ...
        // GET: Vehicles/FreightChargesMaster
        public ActionResult Index(FreightChargesMaVM Model)
        {
            ...
            {
                var mList = ctxTFAT.FreightChargeMa.Where(x => x.DocNo == Model.Document).FirstOrDefault();
                if (mList != null)
                {
                    ...
                    var List = ctxTFAT.FreightChargeMaRef.Where(x => x.DocNo == Model.Document).ToList();
                    ...
                    var Mobj1 = (from TfatBranch in Mobj
                                 select new FreightChargesMaVM
                                 {
                                     ...
                                     Freight = List.Where(x => x.ToBranch == TfatBranch.ID).Select(x => x.Freight).FirstOrDefault() ?? 0,
                                     Advance = List.Where(x => x.ToBranch == TfatBranch.ID).Select(x => x.Advance).FirstOrDefault() ?? 0,
                                     Category = TfatBranch.Category,
                                     ID = TfatBranch.ID,
                                     GRP = TfatBranch.GRP,
                                 }).ToList();
                    ...
                    List<FreightChargesMaVM> tripCharges = new List<FreightChargesMaVM>();
        ...
        public ActionResult TripCharges(FreightChargesMaVM Model)
        {
            ...
            var CheckExist = ctxTFAT.FreightChargeMa.Where(x => x.DocNo != Model.Document && x.Vehicle == Model.Vehicle && x.FromBranch == Model.Branch && x.FromPeriod == Statdate).ToList();
            ...
            {
                bool NewTrip = true;
                var GetTripMaster = ctxTFAT.FreightChargeMa.Where(x => x.DocNo == Model.Document).FirstOrDefault();
                ...
                if (Model.copy)
                {
                    ...
                    var List = ctxTFAT.FreightChargeMaRef.Where(x => x.DocNo == Model.copyDocument && list.Contains(x.ToBranch)).ToList();
                    var Mobj = (from TfatBranch in ctxTFAT.TfatBranch
            ...
            var html = ViewHelper.RenderPartialView(this, "FreightChargesPartial", Model);
            return Json(new { Status = Status, Message = Message, Html = html }, JsonRequestBehavior.AllowGet);
        }

        public ActionResult SaveData(FreightChargesMaVM mModel)
        {
            using (var transaction = ctxTFAT.Database.BeginTransaction())
            {
                try
                {
                    ...
                    FreightChargeMa mobj = new FreightChargeMa();


                    bool mAdd = true;
                    if (ctxTFAT.FreightChargeMa.Where(x => x.DocNo == mModel.Document).FirstOrDefault() != null)
                    {
                        mobj = ctxTFAT.FreightChargeMa.Where(x => x.DocNo == mModel.Document).FirstOrDefault();
                        mAdd = false;
                        DeUpdate(mModel);
                    }
                    if (mAdd)
                    ...
                    foreach (var item in mModel.FreightCharges)
                    {
                        ...
                        {
                            FreightChargeMaRef advRef = new FreightChargeMaRef();
                            ...
                            ctxTFAT.FreightChargeMaRef.Add(advRef);
                        }
                    }
                    if (mAdd == true)
                    {
                        ctxTFAT.FreightChargeMa.Add(mobj);
                    }
                    else
                    {
                    ...
                    transaction.Commit();
                    transaction.Dispose();
                    UpdateAuditTrail(mbranchcode, mModel.Mode, mModel.Header, "     " + mperiod.Substring(0, 2) + mobj.DocNo, DateTime.Now, 0, mobj.DocNo, "Save Freight Charges Master", "NA");
                }
        ...
        public void DeUpdate(FreightChargesMaVM Model)
        {
            var BillSubRef = ctxTFAT.FreightChargeMaRef.Where(x => x.DocNo == Model.Document).ToList();
            ctxTFAT.FreightChargeMaRef.RemoveRange(BillSubRef);
            ctxTFAT.SaveChanges();
        }

        public ActionResult DeleteTripMaster(FreightChargesMaVM mModel)
        {
            ...
            var mList = ctxTFAT.FreightChargeMa.Where(x => (x.DocNo == mModel.Document)).FirstOrDefault();
            ctxTFAT.FreightChargeMa.Remove(mList);
            var BillSubRef = ctxTFAT.FreightChargeMaRef.Where(x => x.DocNo == mModel.Document).ToList();
            ctxTFAT.FreightChargeMaRef.RemoveRange(BillSubRef);
            ctxTFAT.SaveChanges();
            UpdateAuditTrail(mbranchcode, mModel.Mode, mModel.Header, "     " + mperiod.Substring(0, 2) + mList.DocNo, mList.DocDate, 0, mList.DocNo, "Delete Freight Charges Master", "NA");
        ...
        {
            List<CopyVM> List = new List<CopyVM>();

            var Mobj = (from FreightCharge in ctxTFAT.FreightChargeMa
                        where FreightCharge.DocNo != DocNo
                        select new CopyFreightVM
                        {
                            ...
                            FromBranch = ctxTFAT.TfatBranch.Where(x => x.Code == FreightCharge.FromBranch).Select(x => x.Name).FirstOrDefault()
                        }).ToList();
            if (Mobj == null)
            {
                Mobj = new List<CopyFreightVM>();
            }
            var html = ViewHelper.RenderPartialView(this, "ListOFFreightCharge", Mobj);
