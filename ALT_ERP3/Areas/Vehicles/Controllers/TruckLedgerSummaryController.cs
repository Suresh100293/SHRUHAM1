...
{
    public class TruckLedgerSummaryController : BaseController
    {
        ...
        // GET: Vehicles/TruckLedgerSummary
        ...
        public ActionResult GetGridData(TruckLedgerSummaryVM Model)
        {
            ...
            {
                if (Model.ViewDataId == "VLedgerExpSummyDetails")
                {
                    string Query = "  Select * into ztmp_VLedgerExpSummyDetails from (" +
                    ...
                    ExecuteStoredProc("Drop Table ztmp_VLedgerExpSummyDetails");
                ...
                else if (Model.ViewDataId == "VehicleLedgerBalDetails")
                {

                    #region Create TempTable

                    string Query = "create table ztmp_VehicleLedgerBalDetails" +
                                    "  ( " +
                    ...
                    ExecuteStoredProc("Drop Table ztmp_VehicleLedgerBalDetails");
                    SqlConnection tfat_conx = new SqlConnection(GetConnectionString());
                    SqlCommand cmd = new SqlCommand(Query, tfat_conx);
                    ...
                    tbl.Columns.Add(new DataColumn("VehicleNo", typeof(string)));
                    ...
                    for (int i = 0; i < VehicleCode.Length; i++)
                    {
                        ...
                        if (addNewRow)
                        {
                            ...
                            dr["VehicleNo"] = ctxTFAT.Master.Where(y => y.Code == VehicleCodet.ToString()).Select(y => y.Name).FirstOrDefault();
                            if (OpeningBal > 0)
                            {
                    ...
                    objbulk.DestinationTableName = "ztmp_VehicleLedgerBalDetails";
                    objbulk.ColumnMappings.Add("VehicleNo", "VehicleNo");
