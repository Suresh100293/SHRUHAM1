    ...
    public class JournalVoucherController : BaseController
    {
        ...
        public ActionResult Index(LedgerVM Model)
        {
            ...
            else
            {
                ...
                var ledger1 = ctxTFAT.Ledger.Where(x => x.ParentKey == Model.ParentKey && x.Branch == Model.Branch).Select(x => x).FirstOrDefault();
                ...
                Model.NarrStr = ctxTFAT.Narration.Where(x => x.ParentKey == Model.ParentKey && x.Branch == Model.Branch).Select(x => x.Narr).FirstOrDefault() ?? "";
                if (Model.MainType == "JV")
                {
                    Model.ConstantMode = 2;
                }
                ...
                var mobj = (from l in ctxTFAT.Ledger
                            join td in ctxTFAT.TDSPayments on new { l.ParentKey, l.TableKey, l.Branch } equals new { td.ParentKey, td.TableKey, td.Branch } into tds
                            from lefttds in tds.DefaultIfEmpty()
                            where l.ParentKey == Model.ParentKey && l.Branch == Model.Branch
                            select new
                            {
                                l.Branch,
                                l.Code,
                ...
                foreach (var item in mobj)
                {
                    objledgerdetail.Add(new LedgerVM()
                    {
                        ...
                        MainType = Model.MainType,
                        CurrCode = (item.CurrName == null) ? "0" : item.CurrName.ToString(),
                ...
                var ledger = ctxTFAT.Ledger.Where(x => x.ParentKey == Model.ParentKey && x.Branch == Model.Branch).Select(x => x).FirstOrDefault();
                ...
                Model.BalanceAmt = GetBalance(Model.BankCashCode, DateTime.Now.Date, Model.Branch);
                Model.CurrCode = (ledger.CurrName == null) ? "" : ledger.CurrName.ToString();
                ...
                if (Model.CheckMode==false)
                {
                    ...
                    if (!String.IsNullOrEmpty(IntoSingleLine))
                    {
                        ...
                        try
                        {
                            ...
                            {
                                Model.CheckMode = true;
                                Model.Message = "This Document Adjust In TripSheet.. So U Cant Edit/Delete...!<br>";
                            }
                            else
                            {
        ...
        public string CheckValidations(LedgerVM Model)
        {
            ...
            var resultdoctype = ctxTFAT.DocTypes.Where(x => x.Code == "GJV00").Select(x => new { x.BackDays }).FirstOrDefault();

            bool mBackDated = ctxTFAT.UserRightsTrx.Where(x => x.Code == muserid && x.Type == "GJV00").Select(z => z.xBackDated).FirstOrDefault();
            ...
            decimal sumdebit = result.Where(x => x.tempIsDeleted == false).Sum(x => x.Debit);
            decimal sumcredit = result.Where(x => x.tempIsDeleted == false).Sum(x => x.Credit);
            if (sumdebit != sumcredit)
            {
                mMessage = mMessage + "\n" + "Total Debit Not Equal to Total of Credits. Please Check It....!";
            }
        ...
        public ActionResult DeleteData(LedgerVM Model)
        {
            using (var transaction = ctxTFAT.Database.BeginTransaction())
            {
                ...
                {
                    DeUpdate(Model);
                    UpdateAuditTrail(mbranchcode, Model.Mode, Model.Header, Model.ParentKey, DateTime.Now, 0, Model.ParentKey, "Delete JOURNAL VOUCHERS", "NA");
        ...
        private void DeUpdate(LedgerVM Model)
        {
            if (ctxTFAT.TfatBranch.Where(z => z.Code == Model.Branch).Select(x => x.gp_AllowEditDelete).FirstOrDefault() == false)
            {
                ...
                //var mDeleteOS = ctxTFAT.Outstanding.Where(x => x.ParentKey == Model.ParentKey && x.Branch == Model.Branch).ToList();
                ...
                var mDeleteOS2 = ctxTFAT.Outstanding.Where(x => x.ParentKey == Model.ParentKey && x.DocBranch == Model.Branch).ToList();
                ctxTFAT.Outstanding.RemoveRange(mDeleteOS2);

                //foreach (var mled in mDeleteLed)
                //{
                //    var mDeleteOS2 = ctxTFAT.Outstanding.Where(x => x.TableRefKey == mled.TableKey && x.DocBranch == Model.Branch && x.ParentKey == Model.ParentKey ).ToList();
                //    foreach (var item in mDeleteOS2)
                //    {
                //        var mDeleteOS3 = ctxTFAT.Outstanding.Where(x => x.TableKey==item.TableRefKey && x.Srl == item.aSrl && x.aSrl == item.Srl && x.Code==item.Code && x.Amount==item.Amount && x.ParentKey == Model.ParentKey).ToList();
                //        ctxTFAT.Outstanding.RemoveRange(mDeleteOS3);
                //    }
        ...
        public ActionResult SaveData(LedgerVM Model)
        {
            ...
            using (var transaction = ctxTFAT.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (Model.Mode == "Add")
                    {
                        Model.Srl = GetLastSerial("Ledger", mbranchcode, "GJV00", Model.Prefix, Model.SubType, Model.DocDate);
                    ...
                    UpdateAuditTrail(mbranchcode, Model.Mode, Model.Header, Model.ParentKey, DateTime.Now, 0, Model.ParentKey, "Save JOURNAL VOUCHERS", "NA");
                    transaction.Commit();
                    transaction.Dispose();
                    //SendTrnsMsg(Model.Mode, Model.Amt, mbranchcode + Model.ParentKey, ConvertDDMMYYTOYYMMDD(Model.DocuDate), "");
        ...
        public ActionResult AddEditSelectedLedger(LedgerVM Model)
        {
            ...
            {
                Model.OSAdjList = new List<LedgerVM>();
            }
            if (Model.SessionFlag == "Add")
            {
                ...
                if (Model.OSAdjList != null && Model.OSAdjList.Count > 0)
                {
                    ...
                    {
                        item3 = Model.OSAdjList.Select(x => x.BillNumber).Distinct().ToList();
                    }
                    else
                    {
                        item3 = Model.OSAdjList.Select(x => x.Srl).Distinct().ToList();
                    }
                ...
                objledgerdetail.Add(new LedgerVM()
                {
                    ...
                    Narr = Model.Narr,
                    OSAdjList = Model.OSAdjList.GroupBy(x => x.TableKey).Select(y => y.First()).ToList(),
                    Amt = Model.Amt,
            ...
            else
            {
                ...
                {
                    //var DistinctItems = Model.OSAdjList.GroupBy(x => x.TableKey).Select(y => y.First()).ToList();
                    ...
                    item.RefPartyName = ctxTFAT.Master.Where(x => x.Code == Model.RefParty).Select(x => x.Name).FirstOrDefault();
                    ...
                    item.OSAdjList = Model.OSAdjList.GroupBy(x => x.TableKey).Select(y => y.First()).ToList();
                    item.Amt = Model.Amt;
        ...
        public ActionResult GetCashLedger(LedgerVM Model)
        {
            ...
            {
                List<string> item3 = new List<string>();
                List<decimal> item4 = new List<decimal>();
                var result = (List<LedgerVM>)Session["ledgerlist"];
                ...
                foreach (var item in result1)
                {
                    if (item.OSAdjList==null)
                    {
                        item.OSAdjList = new List<LedgerVM>();
                    }
                    item3 = item.OSAdjList.Select(x => x.TableKey).ToList();
                    item4 = item.OSAdjList.Select(x => x.AdjustAmt).ToList();
                    Model.TempDOCD =  String.Join(",", item3);
                    Model.TempDOCDAmt =  String.Join(",", item4);
                    ...
                    Model.RefPartyName = ctxTFAT.Master.Where(x => x.Code == item.RefParty).Select(x => x.Name).FirstOrDefault();
            ...
            else
            {
                ...
                if (Model.MainType == "JV" || Model.MainType == "JM")
                {
                    Model.PaymentMode = "1";
                    Model.PaymentModeName = "Cheque";
                    Model.Amt = result.Sum(x => x.Debit) - result.Sum(x => x.Credit);
                    if (Model.Amt > 0)
                    {
        ...
        public ActionResult CalByEquationAddon(LedgerVM Model)
        {
            ...
            Model.AddOnList = objitemlist;
            var html = ViewHelper.RenderPartialView(this, "~/Areas/Transactions/Views/JournalVoucher/AddOnGrid.cshtml", new LedgerVM() { AddOnList = Model.AddOnList, Mode = Model.Mode, Fld = Model.Fld });
            var jsonResult = Json(new
            {
        ...
        public ActionResult AttachDocument(HttpPostedFileBase files)
        {
            ...
            var html = ViewHelper.RenderPartialView(this, "~/Areas/Accounts/Views/JournalVoucher/AttachmentDocument.cshtml", new LedgerVM() { DocumentList = DocList, AllFileStr = XYZ, FileNameStr = FLN, Mode = "Add" });
        ...
        public ActionResult EditImage(HttpPostedFileBase files, string Document)
        {
            ...
            Session["TempCashBkAttach"] = DocList;
            var html = ViewHelper.RenderPartialView(this, "~/Areas/Accounts/Views/JournalVoucher/AttachmentDocument.cshtml", new LedgerVM() { DocumentList = DocList, AllFileStr = XYZ, FileNameStr = FLN, Mode = "Edit" });
        ...
        public ActionResult DeleteUploadFile(LedgerVM Model)
        {
            ...
            var html = ViewHelper.RenderPartialView(this, "~/Areas/Accounts/Views/JournalVoucher/AttachmentDocument.cshtml", new LedgerVM() { DocumentList = Model.DocumentList, AllFileStr = XYZ, FileNameStr = FLN, Mode = Model.Mode });
        ...
        public List<LedgerVM> GetAttachmentListInEdit(LedgerVM Model)
        {
            ...
            var docdetail = ctxTFAT.Attachment.Where(x => x.ParentKey == Model.ParentKey && x.Branch == Model.Branch).ToList();
        ...
        public ActionResult ConvertMemVoucher(LedgerVM Model)
        {
            using (var transaction = ctxTFAT.Database.BeginTransaction())
            {
                try
                {
                    ...
                    var Moutstanding = ctxTFAT.Outstanding.Where(x => x.ParentKey == Model.ParentKey && x.Type == mType).Select(x => x).ToList();
                    var Moutstanding2 = ctxTFAT.Outstanding.Where(x => x.ParentKey == Model.ParentKey).Select(x => x).ToList();
        ...
        public ActionResult GetMultiPrint(LedgerVM Model)
        {
            ...
            var html = ViewHelper.RenderPartialView(this, "~/Areas/Transactions/Views/JournalVoucher/MultiPrint.cshtml", new LedgerVM() { PrintGridList = Model.PrintGridList, Document = Model.Document });
            var jsonResult = Json(new
            {
                Model.Document,
                Model.PrintGridList,
