...
{
    public class CreditorPaymentSetupController : BaseController
    {
        ...
        private List<SelectListItem> PopulatePrintFormats()
        {
            ...
            var list = ctxTFAT.DocFormats.Where(x => x.Type == "BPM00").OrderBy(x => x.FormatCode).ToList();
        ...
        // GET: Accounts/CreditorPaymentSetup
        public ActionResult Index(CreditorPaymentSetupVM mModel)
        {
            GetAllMenu(Session["ModuleName"].ToString());
            UpdateAuditTrail(mbranchcode, "Setup", mModel.Header, "", DateTime.Now, 0, "", "", "NA");
            mModel.PrintFormats = PopulatePrintFormats();

            var mList = ctxTFAT.CreditorPaymentSetup.Select(x => x).FirstOrDefault();
            if (mList != null)
            {
        ...
        public ActionResult SaveData(CreditorPaymentSetupVM mModel)
        {
            using (var transaction = ctxTFAT.Database.BeginTransaction())
            {
                try
                {
                    CreditorPaymentSetup mobj = new CreditorPaymentSetup();
                    bool mAdd = true;
                    var id = Convert.ToInt64(1);
                    if (ctxTFAT.CreditorPaymentSetup.Select(x => x).FirstOrDefault() != null)
                    {
                        mobj = ctxTFAT.CreditorPaymentSetup.Select(x => x).FirstOrDefault();
                        mAdd = false;
                    }
                    ...
                    mobj.DefaultPrint = "";
                    ExecuteStoredProc("Update DocFormats set Selected='false' where type='BPM00'");
                    ...
                    if (mAdd == true)
                    {
                        ...
                        ctxTFAT.CreditorPaymentSetup.Add(mobj);
                    }
                    else
                    {
                    ...
                    DocTypes docTypes = ctxTFAT.DocTypes.Where(x => x.Code == "BPM00").FirstOrDefault();
                    if (docTypes != null)
                    {
                    ...
                    transaction.Commit();
                    transaction.Dispose();
                    UpdateAuditTrail(mbranchcode, "Setup", mModel.Header, "", DateTime.Now, 0, "", "Save CreditorPayment Setup", "NA");
                }
