...
namespace ALT_ERP3.Areas.Accounts.Controllers
{
    public class ProfitLossClosingController : BaseController
    {
        ...
        public ActionResult SaveData()
        {
            string mparentkey = "";
            decimal mprofit = 0;
            string mdocstring = RecToString("Select Code from DocTypes Where DocBackward<>0").Trim();
            string mserial = "";
            DateTime mDate1 = Convert.ToDateTime(Session["StartDate"]);
            DateTime mDate2 = Convert.ToDateTime(Session["LastDate"]);
            var mList = ctxTFAT.Ledger.Where(x => x.DocDate >= mDate1 && x.DocDate <= mDate2 && x.Type.StartsWith("PAL")).ToList();
            if (mList.Count != 0)
            {
                mserial = ctxTFAT.Ledger.Where(x => x.DocDate >= mDate1 && x.DocDate <= mDate2 && x.Type.StartsWith("PAL")).Select(x => x.Srl).FirstOrDefault() ?? "";
                ctxTFAT.Ledger.RemoveRange(mList);
                ctxTFAT.SaveChanges();
            }
            int mcurrency = ctxTFAT.TfatBranch.Select(z => z.CurrName).FirstOrDefault();
            if (mcurrency == 0)
            {
                mcurrency = 1;//india
            }
            ...
            for (int mpass = 1; mpass <= mnpass; ++mpass)
            {
                using (var transaction = ctxTFAT.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (mserial == "") mserial = GetLastSerial("Ledger", "", mtype, mperiod, "RJ", mDate2);
                        int mCnt = 1;
                        mprofit = 0;
                        ...
                        foreach (string mcode in macclist)
                        {
                            ...
                            if (mpass == 1)
                            {
                                mamt = ctxTFAT.Ledger.Where(x => x.DocDate >= mDate1 && x.DocDate <= mDate2 && x.MainType != "MV" && x.MainType != "PV" && x.Code == mcode && !mdocstring.Contains(x.Type) && x.AUTHORISE.Trim().Substring(0, 3).ToLower() == "A00").Sum(x => x.Debit - x.Credit) ?? 0;
