...
{
    public class OtherTransactionJVSetupController : BaseController
    {
        // GET: Accounts/OtherTransactionJVSetup
        public ActionResult Index(OTransSetup Model)
        {
            GetAllMenu(Session["ModuleName"].ToString());
            UpdateAuditTrail(mbranchcode, "Setup", Model.Header, "", DateTime.Now, 0, "", "", "NA");

            var mOtherTrnsSet = ctxTFAT.OtherTransactSetupJV.Select(x => x).FirstOrDefault();
        ...
        public void DeUpdate(OTransSetup Model)
        {
            ...
            var mobjList = ctxTFAT.OtherTransactSetupJV.FirstOrDefault();
            if (mobjList != null)
            {
                ctxTFAT.OtherTransactSetupJV.Remove(mobjList);
            }
        ...
        public ActionResult SaveData(OTransSetup Model)
        {
            using (var transaction = ctxTFAT.Database.BeginTransaction())
            {
                try
                {
                    ...
                    OtherTransactSetupJV OTSET = new OtherTransactSetupJV();
                    ...
                    ctxTFAT.OtherTransactSetupJV.Add(OTSET);

                    DocTypes docTypes = ctxTFAT.DocTypes.Where(x => x.Code == "COT00").FirstOrDefault();
                    if (docTypes != null)
                    {
                    ...
                    transaction.Commit();
                    transaction.Dispose();
                    UpdateAuditTrail(mbranchcode, "Setup", Model.Header, "", DateTime.Now, 0, "", "Save Other TransactionJV Setup", "NA");
                }
