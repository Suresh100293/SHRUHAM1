...
namespace ALT_ERP3.Areas.Accounts.Controllers
{
    public class OtherTransactionController : BaseController
    {
        ...
        public ActionResult GetTDSRateDetail(OtherTransactModel Model)
        {
        ...
        public ActionResult GetAccountDetails(CreditPurchaseVM Model)
        {
            ...
            var Branch = "";
            var master = ctxTFAT.Master.Where(x => x.Code == Model.BankCashCode).FirstOrDefault();
            Model.RevDoc = "Total Balance :";
            if (master.FetchBalAcc == false)
            {
                Branch = mbranchcode;
                Model.RevDoc = "Branch Balance :";
            }
            string mStr = @"select dbo.GetBalance('" + Model.BankCashCode + "','" + MMDDYY(DateTime.Now) + "','" + Branch + "',0,0)";
        ...
        public ActionResult GetGSTCode(string MainType, string VATGSTApp)
        {
            ...
            var result = ctxTFAT.TaxMaster.Where(x => x.VATGST != false && x.Scope == "P").Select(m => new
            {
                m.Code,
                m.Name
            }).Distinct().ToList();
        ...
        public ActionResult GetRelatedChoiceToList(string term)
        {
            ...
            if (term == "" || term == null)
            {
                var mVehicles = ctxTFAT.Master.Where(x => (mbranchcode.Contains(x.AppBranch) || x.AppBranch == "All") && x.BaseGr != "D" && (!DefaultCost.Contains(x.Code))).OrderBy(n => n.Name).ToList().Take(10);
            ...
            {
                var mVehicles = ctxTFAT.Master.Where(x => (x.AppBranch.Contains(mbranchcode) || x.AppBranch == "All") && x.BaseGr != "D" && (!DefaultCost.Contains(x.Code)) && x.Name.Contains(term)).OrderBy(n => n.Name).ToList().Take(10);
        ...
        public ActionResult GetCashBankList(string term)
        {
            ...
            {
                var result = ctxTFAT.Master.Where(x => (x.AppBranch.Contains(mbranchcode) || x.AppBranch == "All") && x.Name.Contains(term) && (x.BaseGr == "B" || x.BaseGr == "C") && x.Hide == false).Select(m => new { m.Code, m.Name }).Distinct().ToList();
                return Json(result, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult GetAccountList(string term/*, string BaseGr*/)
        {
            if (term == "" || term == null)
            {
                var result = ctxTFAT.Master.Where(x => (x.AppBranch.Contains(mbranchcode) || x.AppBranch == "All") && (x.BaseGr != "D" && x.Name.ToLower().Trim().Contains(term.ToLower().Trim())) && x.Hide == false).Select(m => new
                {
                    m.Code,
                    m.Name
        ...
        public List<OtherTransactModel> GetAttachmentListInEdit(string TableKey)
        {
            List<OtherTransactModel> AttachmentList = new List<OtherTransactModel>();
            var docdetail = ctxTFAT.Attachment.Where(x => x.TableKey == TableKey).ToList();
            foreach (var item in docdetail)
            {
                AttachmentList.Add(new OtherTransactModel()
                {
        ...
        public List<OtherTransactModel> GetLRDetailList(string TableKey)
        {
            List<OtherTransactModel> objledgerdetail2 = new List<OtherTransactModel>();
            ...
            {
                objledgerdetail2.Add(new OtherTransactModel()
                {
                    LRNumber = a.LrNo,
                    LRAmt = a.LrAmt.Value,
                    tempId = a.SrNo,
        ...
        public List<OtherTransactModel> GetFMDetailList(string TableKey)
        {
            List<OtherTransactModel> objledgerdetail2 = new List<OtherTransactModel>();
        ...
        [HttpPost]
        public ActionResult GetTotal(OtherTransactModel Model)
        {
        ...
        public List<AddOns> GetItemWiseData(string TableKey)
        {
            ...
            if (mRelateData != null)
            {
                var mItem = ctxTFAT.ItemMaster.Where(x => x.BaseGr == mRelateData.ProductGroup).Select(x => new { x.Name, x.Code }).OrderBy(x => x.Name).Distinct().ToList();
                var mItemGroup = ctxTFAT.ItemGroups.Where(x => x.Hide == false).Select(x => new { x.Name, x.Code }).Distinct().ToList();
                truckaddonlist.Add(new AddOns()
                {
                    Fld = "F001",
        ...
        public decimal GetVoucherAdjustAmount(int Sno, string Parentkey)
        {
            decimal OSAdj = 0;
            var LedgerKey = ctxTFAT.Ledger.Where(x => x.extraSno == Sno && x.ParentKey == Parentkey).Select(x => x.TableKey).FirstOrDefault();
            if (!String.IsNullOrEmpty(LedgerKey))
            {
                var Ledger = ctxTFAT.Outstanding.Where(x => x.TableRefKey == LedgerKey).ToList();
                if (Ledger != null)
                {
                    if (Ledger.Count() > 0)
                    {
                        OSAdj = Ledger.Sum(x => x.Amount ?? 0);
                    }
                }
            }
        ...
        public ActionResult Index(OtherTransactModel Model)
        {
            ...
            var Setup = ctxTFAT.OtherTransactSetup.FirstOrDefault();
            if (Setup == null)
            {
                Setup = new OtherTransactSetup();
            }
            //var OtherBillQuery = @"select isnull(BillBoth,0) as BillBoth,BillAuto,BackDays,ShowLedgerPost,BackDated,DuplExpLRFMConfirm,NoDuplExpLRFM,ForwardDated,ForwardDays,ShowDocSerial,AllowZeroAmt,AutoRemark,NoDuplExpDt,DuplExpDtConfirm from othertransactsetup";
            ...
            if (Model.Mode == "Add")
            {
                var PreviousDate = Convert.ToString(TempData.Peek("CPOPreviousDate"));
                ...
                Session["OTHERothertrxlist"] = null;
                Session["OTHERTempOthCashBkAttach"] = null;
            ...
            else
            {
                ...
                foreach (var item in mVouchList)
                {
                    ...
                    objledgerdetail.Add(new OtherTransactModel()
                    {
                        ...
                        OSAmt = GetVoucherAdjustAmount(Convert.ToInt32(item.Sno), Model.ParentKey),
                ...
                Session.Add("OTHERothertrxlist", objledgerdetail);
                ...
                Model.Cheque = mLedger.Cheque;
                Model.Narr = mLedger.Narr;
            ...
            var list = ctxTFAT.DocFormats.Where(x => x.Type == "CPO00").Select(x => x).ToList();
            if (list != null)
            {
        ...
        public ActionResult AddCashLedger(OtherTransactModel Model)
        {
            ...
            if (Model.RelatedTo == "000100345" || Model.Code == "000100345")
            {
                ...
                {
                    List<OtherTransactModel> objledgerdetail1 = new List<OtherTransactModel>();
                    if (Session["OTHERothertrxlist"] != null)
                    {
                        objledgerdetail1 = (List<OtherTransactModel>)Session["OTHERothertrxlist"];
                    }
            ...
            if (Model.SessionFlag == "Add")
            {
                ...
                if (Session["OTHERothertrxlist"] != null)
                {
                    objledgerdetail = (List<OtherTransactModel>)Session["OTHERothertrxlist"];
                }
                ...
                if (Setup.AutoRemark)
                {
                    ...
                    if (Model.AddOnList != null)
                    {
                        ...
                        else if (Model.RelatedTo == "000100343" || Model.Code == "000100343")
                        {
                            ...
                            {
                                Model.Narr += " ,Amount : " + Model.ItemList.Where(x => x.Fld == "F006").Select(X => X.ApplCode).FirstOrDefault() + ", Current KM : " + Model.ItemList.Where(x => x.Fld == "F008").Select(X => X.ApplCode).FirstOrDefault() + ", Install  / Received Date : " + Convert.ToDateTime(Model.ItemList.Where(x => x.Fld == "F012").Select(X => X.ApplCode).FirstOrDefault()).ToShortDateString();
                            }
                        }
                        ...
                        {
                            Model.Narr += "Spare Parts Repairs : " + " Receipt No : " + Model.AddOnList.Where(x => x.Fld == "F001").Select(X => X.ApplCode).FirstOrDefault() + ", Receipt Date : " + Convert.ToDateTime(Model.AddOnList.Where(x => x.Fld == "F002").Select(X => X.ApplCode).FirstOrDefault()).ToShortDateString();
                        }
                        ...
                        {
                            Model.Narr += "Tyre :- Type : " + Model.AddOnList.Where(x => x.Fld == "F001").Select(X => X.ApplCode).FirstOrDefault() + ", Place NO : " + Model.AddOnList.Where(x => x.Fld == "F002").Select(X => X.ApplCode).FirstOrDefault() + ", Serial NO : " + Model.AddOnList.Where(x => x.Fld == "F003").Select(X => X.ApplCode).FirstOrDefault();
                        }
                        if (!String.IsNullOrEmpty(Model.RelatedChoice))
                        {
            ...
            {
                var objledgerdetail = (List<OtherTransactModel>)Session["OTHERothertrxlist"];
        ...
        {
            var result = (List<OtherTransactModel>)Session["OTHERothertrxlist"];
            var Status = "Sucess";
            var Item = result.Where(x => x.tempId == Model.tempId).Select(x => x).FirstOrDefault();
            if (Item != null)
            {
                if (Item.OSAmt != 0)
                {
                    ...
                    {
                        Status = "Error",
                        Message = "This Document Adjusted So, Delete Not Allowed....!"
                    }, JsonRequestBehavior.AllowGet);
                }
            }
            var result2 = result.Where(x => x.tempId != Model.tempId).Select(x => x).ToList();
            Session.Add("OTHERothertrxlist", result2);
            ...
            if (Model.TDSFlag)
            {
                TdsAmt = ((sumdebit + sumcredit) * Model.TDSRate) / 100;
            }

            var InvAmt = (sumdebit + sumcredit) + Math.Round(IGST, 2) + Math.Round(SGST, 2) + Math.Round(CGST, 2);
        ...
        public ActionResult GetCashLedger(OtherTransactModel Model)
        {
            ...
            var jsonResult = Json(new { Branch = Branch, Html = this.RenderPartialView("AddEditTransaction", Model) }, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult ConfirmRelationAddEdit(OtherTransactModel Model)
        {
            ...
            if (master.RelatedTo == "LR" && master.ForceCC == true)
            {
                ...
                {
                    Model.LRDetailList = new List<OtherTransactModel>();
                }
            ...
            if (master.RelatedTo == "FM" && master.ForceCC == true)
            {
                ...
                {
                    Model.FMDetailList = new List<OtherTransactModel>();
                }
        ...
        [HttpPost]
        public ActionResult GetLRDetails(OtherTransactModel Model)
        {
            if (Model.LRDetailList != null && Model.LRDetailList.Count() > 0)
            {
        ...
        [HttpPost]
        public ActionResult GetLrMasterDetails(OtherTransactModel Model)
        {
            ...
            {
                Model.ConsignmentExplist = new List<OtherTransactModel>();
            }
        ...
        [HttpPost]
        public ActionResult FetchDocumentList(OtherTransactModel Model)
        {
            List<OtherTransactModel> ValueList = new List<OtherTransactModel>();
            ...
            {
                OtherTransactModel otherTransact = new OtherTransactModel();
        ...
        [HttpPost]
        public ActionResult ConfirmLRSave(OtherTransactModel Model)
        {
            if (Model.SessionFlag == "Add")
            {
                var Setup = ctxTFAT.OtherTransactSetup.FirstOrDefault();
        ...
        public ActionResult AddFMDetails(OtherTransactModel Model)
        {
            List<OtherTransactModel> FMdetaillist = new List<OtherTransactModel>();
            if (Model.SessionFlag == "Add")
            {
                ...
                //    return Json(Model, JsonRequestBehavior.AllowGet);
                //}
                FMdetaillist.Add(new OtherTransactModel()
                {
            ...
            var html = ViewHelper.RenderPartialView(this, "FMDetails", new OtherTransactModel() { FMDetailList = FMdetaillist });
            return Json(new { FMDetailList = FMdetaillist, Html = html, Amt = FMdetaillist.Sum(x => x.FMAmt) }, JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public ActionResult DeleteFMDetails(OtherTransactModel Model)
        {
            var result2 = Model.FMDetailList.Where(x => x.tempId != Model.tempId).Select(x => x).ToList();
            var html = ViewHelper.RenderPartialView(this, "FMDetails", new OtherTransactModel() { FMDetailList = result2 });
            return Json(new { FMDetailList = result2, Html = html, Amt = result2.Sum(x => x.FMAmt) }, JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public ActionResult GetFMDetails(OtherTransactModel Model)
        {
            if (Model.FMDetailList != null && Model.FMDetailList.Count() > 0)
            {
        ...
        [HttpPost]
        public ActionResult GetFMMasterDetails(OtherTransactModel Model)
        {
        ...
        [HttpPost]
        public ActionResult FetchFreightMemoDocumentList(OtherTransactModel Model)
        {
            List<OtherTransactModel> ValueList = new List<OtherTransactModel>();
        ...
        [HttpPost]
        public ActionResult ConfirmFMSave(OtherTransactModel Model)
        {
        ...
        public ActionResult RelatedToDecide(OtherTransactModel Model)
        {
            string PartialDivName = "", html = "", Branch = "", CurrBranch = "Ledger Balance :";
            var mAcc = ctxTFAT.Master.Where(x => x.Code == Model.Code).FirstOrDefault();
            if (mAcc.FetchBalAcc == false)
            {
                Branch = mbranchcode;
                CurrBranch = "Branch Balance :";
            }
            ...
            return Json(new { ShowDriverOrVehicle = Model.ShowDriverOrVehicle, ShowDriverOrVehicleLabel = Model.ShowDriverOrVehicleLabel, AllowToChange = AllowToChange, GSTCode = GSTCode, GSTName = GSTName, GstFlag = GstFlag, IGST = IGST, SGST = SGST, CGST = CGST, CurrBranch = CurrBranch, Balance = Balance, Html = html, PartialDivName = PartialDivName, ReferAccReq = ReferAccReq, CostCenterTally = CostCenterTally }, JsonRequestBehavior.AllowGet);
        }

        public ActionResult ChangeRelatedToDecide(OtherTransactModel Model)
        {
            ...
            return Json(new { Html = html, PartialDivName = PartialDivName, ReferAccReq = ReferAccReq, CostCenterTally = CostCenterTally }, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult GetTruckTypeWiseViewList(OtherTransactModel Model)
        {
            ...
            var result = (List<OtherTransactModel>)Session["OTHERothertrxlist"];
            var result1 = new OtherTransactModel();
            ...
            var html = ViewHelper.RenderPartialView(this, "TruckDetails", new OtherTransactModel() { /*AccReqRelated = Model.AccReqRelated,*/ SaveCostCenter = true, AddOnList = truckaddonlist, RelatedTo = Model.RelatedTo, RelatedChoice = Model.RelatedChoice, PartialDivName = Model.PartialDivName, Code = Model.Code });
            return Json(new { Html = html, RelatedToN = Model.RelatedToN, RelatedTo = Model.RelatedTo }, JsonRequestBehavior.AllowGet);

        }

        [HttpPost]
        public ActionResult GetTruckTypeWiseViewListE(OtherTransactModel Model)
        {
            Model.DocDate = ConvertDDMMYYTOYYMMDD(Model.DocuDate);
            List<AddOns> truckaddonlist = new List<AddOns>();
            bool ExistAddonOrNot = false;
        ...
        [HttpPost]
        public ActionResult GetTruckTypeWiseViewListRelatedToE(OtherTransactModel Model)
        {
            var result = (List<OtherTransactModel>)Session["OTHERothertrxlist"];
            var result1 = new OtherTransactModel();
            Model.DocDate = ConvertDDMMYYTOYYMMDD(Model.DocuDate);
            ...
            var html = ViewHelper.RenderPartialView(this, "TruckDetails", new OtherTransactModel() { /*AccReqRelated = Model.AccReqRelated,*/ SaveCostCenter = true, AddOnList = truckaddonlist, RelatedTo = Model.RelatedTo, RelatedChoice = Model.RelatedChoice, PartialDivName = Model.PartialDivName });
            return Json(new { Html = html, RelatedToN = Model.RelatedToN, RelatedTo = Model.RelatedTo }, JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public ActionResult GetTruckTyreSingleViewList(OtherTransactModel Model)
        {
            Model.DocDate = ConvertDDMMYYTOYYMMDD(Model.DocuDate);
            List<AddOns> truckaddonlist = new List<AddOns>();
            ...
            var html = ViewHelper.RenderPartialView(this, "TruckTableView", new OtherTransactModel() { AddOnList = truckaddonlist, RelatedTo = Model.RelatedTo, SaveCostCenter = true, PartialDivName = Model.PartialDivName });
            return Json(new { Html = html }, JsonRequestBehavior.AllowGet);
        }
        ...
        [HttpPost]
        public ActionResult GetBTNDetail(OtherTransactModel Model)
        {
        ...
        public List<AddOns> GetSessionEditTruckWiseData(OtherTransactModel Model)
        {
        ...
        public List<AddOns> GetSessionEditItemWiseData(OtherTransactModel Model)
        {

            List<AddOns> truckaddonlist = new List<AddOns>();
            var mRelateData = Model.ItemList.Where(x => x.Fld == "F001").Select(x => x.ApplCode).FirstOrDefault();
            var mItem = ctxTFAT.ItemMaster.Where(x => x.BaseGr == mRelateData).Select(x => new { x.Name, x.Code }).OrderBy(x => x.Name).Distinct().ToList();
            var mItemGroup = ctxTFAT.ItemGroups.Where(x => x.Hide == false).Select(x => new { x.Name, x.Code }).Distinct().ToList();
            truckaddonlist.Add(new AddOns()
            {
                Fld = "F001",
        ...
        [HttpPost]
        public ActionResult GetSparePartDetail(OtherTransactModel Model, string KM, string Days)
        {
        ...
        [HttpPost]
        public ActionResult GetTyreDetail(OtherTransactModel Model, string KM, string Days)
        {
            var mSpare = ctxTFAT.ItemMaster.Where(x => x.Code == Model.Type).Select(x => x).FirstOrDefault();
            var mExpdate = DateTime.Now.Date;
            var mDate = Convert.ToDateTime(Model.FromDate);
        ...
        [HttpPost]
        public ActionResult GetCalcTyreDetail(OtherTransactModel Model)
        {
        ...
        public ActionResult GetPostingNew(OtherTransactModel Model)
        {
            ...
            List<OtherTransactModel> LedPostList = new List<OtherTransactModel>();
            int mCnt = 1;
            var objledgerdetail = (List<OtherTransactModel>)Session["OTHERothertrxlist"];
            ...
            {
                var resultw = ctxTFAT.DocTypes.Where(x => x.Code == "CPO00").Select(x => x).FirstOrDefault();
            ...
            if (ctxTFAT.PeriodLock.Where(x => x.Branch == mbranchcode && x.Type == Model.Type && x.LockDate == Date).FirstOrDefault() != null)
            {
                return Json(new
                {
            ...
            foreach (var item in objledgerdetail)
            {
                if (item.AmtType == "Payment")
                {
                    LedPostList.Add(new OtherTransactModel()
                    {
                            ...
                            if (taxMaster != null)
                            {
                                ...
                                if (item.IGSTRate > 0)
                                {
                                    LedPostList.Add(new OtherTransactModel()
                                    {
                                        ...
                                        AccountName = ctxTFAT.Master.Where(x => x.Code == Model.BankCashCode).Select(x => x.Name).FirstOrDefault(),
                                        Debit = 0,
                                        Credit = Math.Round((item.Amt * item.IGSTRate) / 100, 2),
                                        Branch = mbranchcode,
                                        BranchName = ctxTFAT.TfatBranch.Where(X => X.Code == mbranchcode).Select(x => x.Name).FirstOrDefault(),
                                        tempId = mCnt,
                                        Narr = item.Narr,
                                    ...
                                    LedPostList.Add(new OtherTransactModel()
                                    {
                                        ...
                                        Debit = Math.Round((item.Amt * item.IGSTRate) / 100, 2),
                                        Credit = 0,
                                        Branch = mbranchcode,
                                        BranchName = ctxTFAT.TfatBranch.Where(X => X.Code == mbranchcode).Select(x => x.Name).FirstOrDefault(),
                                        tempId = mCnt,
                                ...
                                if (item.CGSTRate > 0)
                                {
                                    LedPostList.Add(new OtherTransactModel()
                                    {
                                        ...
                                        AccountName = ctxTFAT.Master.Where(x => x.Code == Model.BankCashCode).Select(x => x.Name).FirstOrDefault(),
                                        Debit = 0,
                                        Credit = Math.Round((item.Amt * item.CGSTRate) / 100, 2),
                                        Branch = mbranchcode,
                                        BranchName = ctxTFAT.TfatBranch.Where(X => X.Code == mbranchcode).Select(x => x.Name).FirstOrDefault(),
                                        tempId = mCnt,
                                        Narr = item.Narr,
                                    ...
                                    LedPostList.Add(new OtherTransactModel()
                                    {
                                        ...
                                        Debit = Math.Round((item.Amt * item.CGSTRate) / 100, 2),
                                        Credit = 0,
                                        Branch = mbranchcode,
                                        BranchName = ctxTFAT.TfatBranch.Where(X => X.Code == mbranchcode).Select(x => x.Name).FirstOrDefault(),
                                        tempId = mCnt,
                                ...
                                if (item.SGSTRate > 0)
                                {
                                    LedPostList.Add(new OtherTransactModel()
                                    {
                                        ...
                                        AccountName = ctxTFAT.Master.Where(x => x.Code == Model.BankCashCode).Select(x => x.Name).FirstOrDefault(),
                                        Debit = 0,
                                        Credit = Math.Round((item.Amt * item.SGSTRate) / 100, 2),
                                        Branch = mbranchcode,
                                        BranchName = ctxTFAT.TfatBranch.Where(X => X.Code == mbranchcode).Select(x => x.Name).FirstOrDefault(),
                                        tempId = mCnt,
                                        Narr = item.Narr,
                                    ...
                                    LedPostList.Add(new OtherTransactModel()
                                    {
                                        ...
                                        Debit = Math.Round((item.Amt * item.SGSTRate) / 100, 2),
                                        Credit = 0,
                                        Branch = mbranchcode,
                                        BranchName = ctxTFAT.TfatBranch.Where(X => X.Code == mbranchcode).Select(x => x.Name).FirstOrDefault(),
                                        tempId = mCnt,
                        ...
                        if (Model.TDSRate > 0)
                        {
                            ...
                            //if (!String.IsNullOrEmpty(PostAC))
                            {
                                LedPostList.Add(new OtherTransactModel()
                                {
                                    ...
                                    AccountName = ctxTFAT.Master.Where(x => x.Code == Model.BankCashCode).Select(x => x.Name).FirstOrDefault(),
                                    Debit = 0,
                                    Credit = Math.Round((item.Amt * Model.TDSRate) / 100, 2),
                                    Branch = mbranchcode,
                                    BranchName = ctxTFAT.TfatBranch.Where(X => X.Code == mbranchcode).Select(x => x.Name).FirstOrDefault(),
                                    tempId = mCnt,
                                    Narr = item.Narr,
                                ...
                                LedPostList.Add(new OtherTransactModel()
                                {
                                    ...
                                    Debit = Math.Round((item.Amt * Model.TDSRate) / 100, 2),
                                    Credit = 0,
                                    Branch = mbranchcode,
                                    BranchName = ctxTFAT.TfatBranch.Where(X => X.Code == mbranchcode).Select(x => x.Name).FirstOrDefault(),
                                    tempId = mCnt,
                ...
                else if (item.AmtType == "Receipt")
                {
                    ...
                    if (item.GSTFlag)
                    {
                        if (!String.IsNullOrEmpty(item.GSTCode))
                        {
                            ...
                            if (taxMaster != null)
                            {
                                string SGSTOutputAccount = "000100035", CGSTOutputAccount = "000100036", IGSTOutputAccount = "000100037";
                                if (item.IGSTRate > 0)
                                {
                                    LedPostList.Add(new OtherTransactModel()
                                    {
                                        ...
                                        tempId = mCnt,
                                        Narr = item.Narr,
                                        AltCode = IGSTOutputAccount,
                                    });
                                    mCnt = mCnt + 1;
                                    LedPostList.Add(new OtherTransactModel()
                                    {
                                        Code = IGSTOutputAccount,
                                        AccountName = ctxTFAT.Master.Where(x => x.Code == IGSTOutputAccount).Select(x => x.Name).FirstOrDefault(),
                                        Debit = 0,
                                        Credit = Math.Round((item.Amt * item.IGSTRate) / 100, 2),
                                        Branch = mbranchcode,
                                        BranchName = ctxTFAT.TfatBranch.Where(X => X.Code == mbranchcode).Select(x => x.Name).FirstOrDefault(),
                                ...
                                if (item.CGSTRate > 0)
                                {
                                    LedPostList.Add(new OtherTransactModel()
                                    {
                                        ...
                                        tempId = mCnt,
                                        Narr = item.Narr,
                                        AltCode = CGSTOutputAccount,
                                    });
                                    mCnt = mCnt + 1;
                                    LedPostList.Add(new OtherTransactModel()
                                    {
                                        Code = CGSTOutputAccount,
                                        AccountName = ctxTFAT.Master.Where(x => x.Code == CGSTOutputAccount).Select(x => x.Name).FirstOrDefault(),
                                        Debit = 0,
                                        Credit = Math.Round((item.Amt * item.CGSTRate) / 100, 2),
                                        Branch = mbranchcode,
                                        BranchName = ctxTFAT.TfatBranch.Where(X => X.Code == mbranchcode).Select(x => x.Name).FirstOrDefault(),
                                ...
                                if (item.SGSTRate > 0)
                                {
                                    LedPostList.Add(new OtherTransactModel()
                                    {
                                        ...
                                        tempId = mCnt,
                                        Narr = item.Narr,
                                        AltCode = SGSTOutputAccount,
                                    });
                                    mCnt = mCnt + 1;
                                    LedPostList.Add(new OtherTransactModel()
                                    {
                                        Code = SGSTOutputAccount,
                                        AccountName = ctxTFAT.Master.Where(x => x.Code == SGSTOutputAccount).Select(x => x.Name).FirstOrDefault(),
                                        Debit = 0,
                                        Credit = Math.Round((item.Amt * item.SGSTRate) / 100, 2),
                                        Branch = mbranchcode,
                                        BranchName = ctxTFAT.TfatBranch.Where(X => X.Code == mbranchcode).Select(x => x.Name).FirstOrDefault(),
                                ...
                                if (hSNMaster != null)
                                {
                                    if (item.IGSTRate > 0)
                                    {
                                        LedPostList.Add(new OtherTransactModel()
                                        {
                                            ...
                                            tempId = mCnt,
                                            Narr = item.Narr,
                                            AltCode = hSNMaster.IGSTOut,
                                        });
                                        mCnt = mCnt + 1;
                                        LedPostList.Add(new OtherTransactModel()
                                        {
                                            Code = hSNMaster.IGSTOut,
                                            AccountName = ctxTFAT.Master.Where(x => x.Code == hSNMaster.IGSTOut).Select(x => x.Name).FirstOrDefault(),
                                            Debit = 0,
                                            Credit = Math.Round((item.Amt * item.IGSTRate) / 100, 2),
                                            Branch = mbranchcode,
                                            BranchName = ctxTFAT.TfatBranch.Where(X => X.Code == mbranchcode).Select(x => x.Name).FirstOrDefault(),
                                    ...
                                    if (item.CGSTRate > 0)
                                    {
                                        LedPostList.Add(new OtherTransactModel()
                                        {
                                            ...
                                            tempId = mCnt,
                                            Narr = item.Narr,
                                            AltCode = hSNMaster.CGSTOut,
                                        });
                                        mCnt = mCnt + 1;
                                        LedPostList.Add(new OtherTransactModel()
                                        {
                                            Code = hSNMaster.CGSTOut,
                                            AccountName = ctxTFAT.Master.Where(x => x.Code == hSNMaster.CGSTOut).Select(x => x.Name).FirstOrDefault(),
                                            Debit = 0,
                                            Credit = Math.Round((item.Amt * item.CGSTRate) / 100, 2),
                                            Branch = mbranchcode,
                                            BranchName = ctxTFAT.TfatBranch.Where(X => X.Code == mbranchcode).Select(x => x.Name).FirstOrDefault(),
                                    ...
                                    if (item.SGSTRate > 0)
                                    {
                                        LedPostList.Add(new OtherTransactModel()
                                        {
                                            ...
                                            tempId = mCnt,
                                            Narr = item.Narr,
                                            AltCode = hSNMaster.SGSTOut,
                                        });
                                        mCnt = mCnt + 1;
                                        LedPostList.Add(new OtherTransactModel()
                                        {
                                            Code = hSNMaster.SGSTOut,
                                            AccountName = ctxTFAT.Master.Where(x => x.Code == hSNMaster.SGSTOut).Select(x => x.Name).FirstOrDefault(),
                                            Debit = 0,
                                            Credit = Math.Round((item.Amt * item.SGSTRate) / 100, 2),
                                            Branch = mbranchcode,
                                            BranchName = ctxTFAT.TfatBranch.Where(X => X.Code == mbranchcode).Select(x => x.Name).FirstOrDefault(),
        ...
        public void DeUpdate(OtherTransactModel Model)
        {
            ...
            var result1 = (List<OtherTransactModel>)Session["OTHERothertrxlist"];
            foreach (var item in result1)
            {
            ...
            if (Model.Mode == "Delete")
            {
                ...
                List<OtherTransactModel> result = new List<OtherTransactModel>();
                result = (List<OtherTransactModel>)Session["OTHERothertrxlist"];
                foreach (var item in result)
                {
        ...
        public ActionResult SaveData(OtherTransactModel Model)
        {
            ...
            using (var transaction = ctxTFAT.Database.BeginTransaction(System.Data.IsolationLevel.Unspecified))
            {
                try
                {
                    ...
                    TempData["CPOPreviousDate"] = Model.DocuDate;
                    if (Model.Mode == "Add")
                    {
                        if (Model.OTGenerate == "M")
                        {
                            var result1 = ctxTFAT.DocTypes.Where(x => x.Code == "CPO00").Select(x => x).FirstOrDefault();
                    ...
                    TfatUserAuditHeader authorisation = ctxTFAT.TfatUserAuditHeader.Where(x => x.Type == "CPO00").FirstOrDefault();
                    ...
                    OtherTransactSetup setup = ctxTFAT.OtherTransactSetup.FirstOrDefault();
                    if (setup == null)
                    {
                        setup = new OtherTransactSetup();
                    }
                    if (Model.Mode == "Add" || Model.Mode == "Edit")
                    {
                        ...
                        List<Ledger> ledgerslist = new List<Ledger>();
                        ...
                        for (int u = 0; u < ledpost.Count; u++)
                        {
                            ...
                            mobjL.TDSCode = Convert.ToInt32(Model.TDSCode);
                        ...
                        if (result != null)
                        {
                            ...
                            foreach (var li in list)
                            {
                                ...
                                //reldt.ReqRelated = li.AccReqRelated;

                                var GetLedger = ledgerslist.OrderBy(x => x.Sno).Where(x => x.Code == reldt.Code && x.Debit + x.Credit == reldt.Amount && x.extraSno == 0).FirstOrDefault();
                                if (GetLedger != null)
                                {
                                    GetLedger.extraSno = xCnt;
                                }
                        ...
                        ctxTFAT.Ledger.AddRange(ledgerslist);
                        #endregion
                        ...
                        UpdateAuditTrail(Model.Branch, Model.Mode, Model.Header, Model.ParentKey, ConvertDDMMYYTOYYMMDD(Model.DocuDate), Model.Amt, Model.BankCashCode, "Save Cash Bank Transaction", "A");
        ...
        private void SaveTDSPayments(OtherTransactModel Model)
        {
            var TDSCODE = ctxTFAT.TDSMaster.Where(x => x.Code.ToString() == Model.TDSCode).FirstOrDefault();
            var tdsrates = TDSCODE == null ? null : ctxTFAT.TDSRates.Where(x => x.Code == TDSCODE.Code).FirstOrDefault();
            if (TDSCODE != null)
            {
                if (Model.TDSFlag == true)
                {
                    ...
                    tspay.TDSAble = 0;
                    tspay.TDSAmt = Convert.ToDecimal(Model.TDSAmt);
                    tspay.TDSCess = tdsrates == null ? 0 : tdsrates.Cess;
                    tspay.TDSCessAmt = 0;
                    tspay.TDSCode = TDSCODE.Code;
                    tspay.TDSReason = 0;
                    ...
                    tspay.TDSTax = tdsrates == null ? 0 : Model.TDSRate;
                    tspay.TDSTaxAmt = 0;
                    tspay.TotalTDSAmt = Convert.ToDecimal(Model.TDSAmt);
                    ...
                    var mBalCntNoCred = Model.LedgerPostList.Where(x => x.Code == Model.BankCashCode && x.Credit == tspay.TotalTDSAmt).Select(x => x.tempId).FirstOrDefault();
                    ...
                    osobj1.aSno = 2;
                    osobj1.Amount = Convert.ToDecimal(Model.TDSAmt);
                    ...
                    osobj1.BillNumber = Model.BillNumber == null ? " " : Model.BillNumber;
                    ...
                    osobj1.TDSAmt = Convert.ToDecimal(Model.TDSAmt);
                    osobj1.ENTEREDBY = muserid;
                    osobj1.LASTUPDATEDATE = DateTime.Now;
                    osobj1.CurrAmount = Convert.ToDecimal(Model.TDSAmt);
                    ...
                    osobj2.TDSAmt = Convert.ToDecimal(Model.TDSAmt);
                    osobj2.ENTEREDBY = muserid;
                    osobj2.LASTUPDATEDATE = DateTime.Now;
                    osobj2.CurrAmount = Convert.ToDecimal(Model.TDSAmt);
        ...
        public ActionResult DeleteData(OtherTransactModel Model)
        {
            ...
            var mobj11 = ctxTFAT.Ledger.Where(x => x.ParentKey == Model.ParentKey && x.Branch == Model.Branch).Select(x => x.Branch + x.TableKey).ToList();
            if (mobj11 != null)
            {
                string IntoSingleLine = "";
                IntoSingleLine = String.Join(",", mobj11);
                //foreach (var item in mobj11)
                //{
                //    IntoSingleLine += "" + item + ",";
                //}
                if (!String.IsNullOrEmpty(IntoSingleLine))
            ...
            using (var transaction = ctxTFAT.Database.BeginTransaction(System.Data.IsolationLevel.Unspecified))
            {
                try
                {
                    var SetupTyreStockDelete = ctxTFAT.OtherTransactSetup.Select(x => x.TyreStockDelete).FirstOrDefault();
                    ...
                    DeUpdate(Model);
                    UpdateAuditTrail(mbranchcode, "Delete", Model.Header, Model.ParentKey, ConvertDDMMYYTOYYMMDD(Model.DocuDate), Model.Amt, Model.BankCashCode, "Delete Cash Bank Transaction", "A");
        ...
        public void SaveAttachment(AttachmentVM Model)
        {
            ...
            if (DocList != null && DocList.Count != 0)
            {
                ...
                foreach (var item in DocList.ToList())
                {
                    ...
                    att.Type = String.IsNullOrEmpty(item.Type) == true ? "CPO00" : item.Type;
