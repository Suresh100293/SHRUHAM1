    ...
    public class ARAPInterfaceController : BaseController
    {
        ...
        public ActionResult GetParties(string term)
        {
            ...
            if (mP1 == "")
            {
                ...
                //return Json(result, JsonRequestBehavior.AllowGet);
                return Json(GetDataTableList("Select top 10 Code, Name from Master Where (BaseGr = '" + mP2 + "' or BaseGr = 'U') and Hide=0 and Charindex('" + term + "',Name)<>0 Order by Name"), JsonRequestBehavior.AllowGet);
            }
            else
            {
                return Json(GetDataTableList("Select top 10 Code, Name from Master Where (BaseGr = '" + mP2 + "' or BaseGr='U') and Hide=0 and Charindex('" + mP1 + "',Name)<>0 Order by Name"), JsonRequestBehavior.AllowGet);
        ...
        public ActionResult GetGridDataDebit(GridOption Model)
        {
            ...
            return GetGridReport(Model, "R", "Code^" + Model.Code + "~Warehouse^" + Model.LocationCode + "~DrCrFlag^" + Flag + "~IncludeZero^" + Model.IsFormatSelected);
        ...
        public ActionResult FIFOAdjust(GridOption Model)
        {
            ...
            string mStr = "Select MainType,SubType,Code,TableKey,Sno,Type,Prefix,Srl,(Outstanding.Debit + Outstanding.Credit) - isnull((Select Sum(o.Amount) from Outstanding o Where o.TableRefKey = Outstanding.TableKey ),0) as Pending " +
                            "from Ledger Outstanding Where Outstanding.Code='" + Model.Code + "' And " + (Model.MainType == "SL" ? "Outstanding.Debit" : "Outstanding.Credit") +
                            " <> 0 And Outstanding.MainType<>'MV' And Outstanding.MainType <> 'PV' And " +
                            "(Outstanding.Debit + Outstanding.Credit) - isnull((Select Sum(o.Amount) from Outstanding o " +
                            "where o.TableRefKey = Outstanding.TableKey  " +
                            " ),0)<> 0 " + (string.IsNullOrEmpty(mDocString) ? "" : " And CharIndex(Outstanding.Type,'" + mDocString + "')=0") +
                            "And OutStanding.DocDate <= '" + MMDDYY(Convert.ToDateTime(Model.ToDate)) + "' And Left(Authorise,1) = 'A'  " +
                            "  Order by Outstanding.DocDate";
            SqlConnection conn = new SqlConnection(GetConnectionString());
            SqlDataAdapter dadr = new SqlDataAdapter(mStr, conn);
            dadr.Fill(griddebitDt);

            mStr = "Select ParentKey,MainType,SubType,Code,TableKey,Sno,Type,Prefix,Srl,(Outstanding.Debit + Outstanding.Credit) - isnull((Select Sum(o.Amount) from Outstanding o Where o.TableRefKey = Outstanding.TableKey ),0) as Pending " +
                "from Ledger Outstanding Where Outstanding.Code = '" + Model.Code + "' And  Outstanding.MainType <> 'MV' " +
                "And Outstanding.MainType <> 'PV'  And(Outstanding.Debit + Outstanding.Credit) - isnull((Select Sum(o.Amount) from Outstanding o " +
                "where o.TableRefKey = Outstanding.TableKey " + (string.IsNullOrEmpty(mDocString) ? "" : " And CharIndex(Type,'" + mDocString + "')=0 ") +
                "),0)<> 0 And OutStanding.DocDate <= '" + MMDDYY(Convert.ToDateTime(Model.ToDate)) + "' And " + (Model.MainType == "SL" ? "Outstanding.Credit" : "Outstanding.Debit") +
                "<>0 And Left(Authorise,1) = 'A'  " +
                "Order by Outstanding.DocDate";
                    ...
                    foreach (DataRow mdrow in griddebitDt.Rows)
                    {
                        ...
                        foreach (DataRow mcrow in gridcreditDt.Rows)
                        {
                            ...
                            if (mcamt != 0 && mcamt > 0)
                            {
                                ...
                                #region Debtor Receipt
                                var tablekeyBRC = mcrow["TableKey"].ToString();
                                Ledger ledgerBRC = ctxTFAT.Ledger.Where(x => x.Code == partycode && x.TableKey == tablekeyBRC).FirstOrDefault();
                                var DebtorReceiptList = ctxTFAT.DebtorRecVouDetail.Where(x => x.Branch == ledgerBRC.Branch && x.ParentKey == ledgerBRC.ParentKey).ToList();
                                int mCnt = DebtorReceiptList.Count() + 1;
                                #endregion


                                if (mcrow["Type"].ToString() == "BRC00")
                                {
                                    DebtorRecVouDetail vd = new DebtorRecVouDetail();
                                    vd.Amount = mamt;
                                    vd.Branch = ledgerBRC.Branch;

                                    vd.Sno = mCnt;
                                    vd.Srl = ledgerBRC.Srl;
                                    ...
                                    vd.Val9 = 0;
                                    vd.Val10 = 0;
                                    vd.TableKey = ledgerBRC.Branch + "BRC00" + ledgerBRC.Prefix.Substring(0, 2) + mCnt.ToString("D3") + ledgerBRC.Srl;
                                    vd.ParentKey = ledgerBRC.Branch + "BRC00" + ledgerBRC.Prefix.Substring(0, 2) + ledgerBRC.Srl;
                                    vd.Prefix = ledgerBRC.Prefix;
                                    vd.NetAmt = mamt;
                                    vd.Type = "BRC00";
                                    vd.InvBranch = ledger.Branch;
                                    vd.InvTableKey = ledger.TableKey;
                                    vd.InvPrefix = ledger.Prefix;
                                    vd.InvType = ledger.Type;
                                    vd.InvSrl = ledger.Srl;
                                    vd.InvSno = ledger.Sno;
                                    vd.TdsAmout = 0;
                                    vd.AutoAdjInterface = true;
                                    ctxTFAT.DebtorRecVouDetail.Add(vd);
                                    ctxTFAT.SaveChanges();
                                    //trxfifo.Commit();
                                }
                                else if (mcrow["Type"].ToString() == "BPM00")
                                {
                                    ...
                                    vd.Val9 = 0;
                                    vd.Val10 = 0;
                                    vd.TableKey = ledgerBRC.Branch + "BPM00" + ledgerBRC.Prefix.Substring(0, 2) + mCnt.ToString("D3") + ledgerBRC.Srl;
                                    vd.ParentKey = ledgerBRC.Branch + "BPM00" + ledgerBRC.Prefix.Substring(0, 2) + ledgerBRC.Srl;
                                    vd.Prefix = ledgerBRC.Prefix;
                                    vd.NetAmt = mamt;
                                    vd.Type = "BPM00";
                                    vd.InvBranch = ledger.Branch;
                                ...
                                os.DocBranch = ledgerBRC.Branch;
                                os.Code = partycode;
                                os.ParentKey = mcrow["ParentKey"].ToString(); ;
                                os.TableKey = ledger.TableKey; //ledger.Type + ledger.Prefix.Substring(0, 2) + ledger.Sno.ToString("D3") + ledger.Srl;
                                ...
                                os2.DocBranch = ledgerBRC.Branch;
                                os2.Code = partycode;
        ...
        public ActionResult ReleaseAdjust(List<string> CTablekey, List<string> DTablekey, string Code, string LocationCode,string MainType)
        {
            if (DTablekey != null)
            {
                if (DTablekey.Count > 0)
                {
                    foreach (var item in DTablekey)
                    {
                        if (MainType=="SL")
                        {
                            ...
                            ExecuteStoredProc("Delete from DebtorRecVouDetail Where InvTableKey='" + item + "' ");
                        }
                        else
                        {
                            ExecuteStoredProc("Delete from Outstanding Where TableKey='" + item + "' or TableRefKey='" + item + "'");
                            ExecuteStoredProc("Delete from CreditorRecVouDetail Where InvTableKey='" + item + "' ");
                        }
            ...
            if (CTablekey != null)
            {
                if (CTablekey.Count > 0)
                {
                    foreach (var item in CTablekey)
                    {
                        ...
                        {
                            ExecuteStoredProc("Delete from Outstanding Where TableKey='" + item + "' or TableRefKey='" + item + "'");
                            ExecuteStoredProc("Delete from DebtorRecVouDetail Where Parentkey =(select L.Parentkey from ledger L where L.Tablekey='" + item + "')");
                        }
                        else
                        {
                            ExecuteStoredProc("Delete from Outstanding Where TableKey='" + item + "' or TableRefKey='" + item + "'");
                            ExecuteStoredProc("Delete from CreditorRecVouDetail Where Parentkey =(select L.Parentkey from ledger L where L.Tablekey='" + item + "')");
                        }
                        //var mosdel = ctxTFAT.Outstanding.Where(z => z.TableKey == item || z.TableRefKey == item).ToList();
                        //if (mosdel != null)
                        //{
        ...
        public ActionResult Adjust(List<string> CTablekey, List<string> DTablekey, string PartyCode, string EndDate, string LocationCode, string MainType)
        {
            ...
            if (DTablekey.Count > 0)
            {
               ...
               "Abs(CurrAmount) as [Curr.Amt],CurrName as Curr,Narr as Narration from Ledger Outstanding Where Outstanding.Code = '" + PartyCode + "' and " + (MainType == "SL" ? "Outstanding.Debit" : "Outstanding.Credit") +
               " <> 0 And Outstanding.MainType <> 'MV' And Outstanding.MainType <> 'PV' And " +
               "(Outstanding.Debit + Outstanding.Credit) - isnull((Select Sum(o.Amount) from Outstanding o " +
               "where o.Code = Outstanding.Code And o.TableRefKey = Outstanding.TableKey),0)<> 0 And " +
               "OutStanding.DocDate <= '" + EndDate + "' And  TableKey in (" + TabKey + ") Order by Outstanding.DocDate";
                //And Left(Authorise,1) = 'A' => SDS 5.8.2021 Removed to enable adj of RejectedEntries
                SqlConnection conn = new SqlConnection(GetConnectionString());
            ...
            if (CTablekey.Count > 0)
            {
               ...
               "CurrName,Narr from Ledger Outstanding Where Outstanding.Code = '" + PartyCode + "' and " + (MainType == "SL" ? "Outstanding.Credit" : "Outstanding.Debit") +
               " <> 0 And Outstanding.MainType <> 'MV' " +
               "And Outstanding.MainType <> 'PV'  And(Outstanding.Debit + Outstanding.Credit) - isnull((Select Sum(o.Amount) from Outstanding o " +
