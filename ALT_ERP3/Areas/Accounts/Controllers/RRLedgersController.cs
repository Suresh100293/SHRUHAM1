...
{
    public class RRLedgersController : BaseController
    {
        ...
        public ActionResult GetAccountList(string term)
        {
            if (mbasegr == "CB")
            {
                if (term == "")
                {
                    ...
                    return Json(GetDataTableList("Select Top 10 Code,Name from Master where (BaseGr='B' or BaseGr='C') and Hide=0 Order by Name"), JsonRequestBehavior.AllowGet);
                ...
                else
                {
                    ...
                    return Json(GetDataTableList("Select Top 10 Code,Name from Master where Name like '%" + term + "%' and (BaseGr='B' or BaseGr='C') and Hide=0 Order by Name"), JsonRequestBehavior.AllowGet);
            ...
            else
            {
                if (term == "")
                {
                    ...
                    return Json(GetDataTableList("Select Top 10 Code, Name = '[' + Code + '] ' + isnull(ShortName,'') + ' ' + Name + ' ' + ' ('+isnull(BaseGr,'')+') ' + (case when basegr='D' or Basegr='S' then isnull(City,'') else '' end) from Master Where BaseGr<>'C' and BaseGr <> 'B' and Hide=0 Order by Name"), JsonRequestBehavior.AllowGet);
                ...
                else
                {
                    ...
                    return Json(GetDataTableList("Select Top 10 Code, Name = '[' + Code + '] ' + isnull(ShortName,'') + ' ' + Name + ' ' + ' ('+isnull(BaseGr,'')+') ' + (Case when basegr='D' or Basegr='S' then isnull(City,'') else '' end) from Master Where BaseGr <> 'C' and BaseGr <> 'B' and Hide=0 and Charindex('" + term + "', Name)<>0 Order by Name"), JsonRequestBehavior.AllowGet);
