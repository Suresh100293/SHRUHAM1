    ...
    public class GSTR1Controller : BaseController
    {
        ...
        public ActionResult GetGridData(GridOption Model)
        {
            ...
            if (mcode != "b2b" && mcode != "AT" && mcode != "atadj" && mcode != "hsn")
            {
                ...
                mfrom += " left join SalesMore r on s.ParentKey = r.TableKey";
                mfrom += " left join Purchase p on p.TableKey = l.TableKey";
                mfrom += " Left Join Address a on l.Code = a.Code And l.AltAddress = a.Sno";
                ...
                mfrom += " where " + (string.IsNullOrEmpty(mDocString) ? "" : "CharIndex(t.Type,'" + mDocString + "')=0 And ");
                msumstring += mfrom;
                mSQL += mfrom;
            ...
            switch (mcode)
            {
                case "b2b":
                    ...
                    mSQL += "Select(Case when s.SubType <> 'CS' then a.GSTNo else (Select m.GSTNo from Address m where m.Code = r.RefParty And m.Sno = 0) End) as [GSTIN / UIN of Recipient], ";
                    mSQL += "s.Branch+s.ParentKey as [Document], ";
                    mSQL += "a.Name as [Receiver Name], ";
                    mSQL += (chkPrefixType == true ? "t.Type+'-'+" : "") + "t.Srl as [Invoice Number], ";
                    mSQL += "Replace(Convert(nVARCHAR, t.DocDate, 106), ' ', '-') as [Invoice date], ";
                    ...
                    mSQL += "(Case When s.GSTType = 1 or s.GSTType = 6 then 'Y' Else 'N' End) as [Reverse Charge], ";
                    mSQL += "0 as [Applicalble %of TaxRate], ";   // abatement from taxmaster goes here
                    mSQL += "(Case s.GSTType When 7 then 'SEZ supplies with payment' When 8 then 'SEZ supplies without payment' When 5 then 'Deemed Exp' Else 'Regular' End) as [Invoice Type], ";
                    mSQL += "'' as [E-Commerce GSTIN], ";
                    mSQL += "Cast(s.IGSTRate + s.cGSTRate + s.sGSTRate as Decimal(15, 2)) as [Rate], ";
                    mSQL += "Cast(s.Taxable*-1 as Decimal(15, 2)) as [Taxable Value], ";
                    mSQL += "Cast(0 as Decimal(15, 2)) as [Cess Amount], ";
                    mSQL += "Cast(abs(s.IGSTAmt + s.cGSTAmt + s.sGSTAmt) as Decimal(15, 2)) as [GST Amount] ";
                    mSQL += "from StockTax s ";
                    mSQL += "Left join Stock t on s.tablekey = t.tablekey ";
                    mSQL += "Left join Sales l on s.ParentKey = l.TableKey ";
                    mSQL += "Left join SalesMore r on s.ParentKey = r.TableKey ";
                    mSQL += "Left join Purchase p on p.TableKey = l.TableKey ";
                    mSQL += "Left Join Address a on s.Party = a.Code And s.AltAddress = a.Sno ";
                    mSQL += "Left Join TfatState x on x.Name = l.PlaceOfSupply ";
                    mSQL += "Where (s.SubType = 'JS' or s.SubType = 'RS' or s.SubType = 'CS' ";
                    mSQL += (chkShowPurRet == true ? " or s.SubType='NP'" : "");
                    mSQL += ") And (Case when s.SubType<>'CS' then a.GSTNo else (Select Top 1 m.GSTNo from Address m where m.Code=r.RefParty And m.Sno=0) End)<>''  And (s.GSTType=0 or s.GSTType=7 or s.GSTType=8 or s.GSTType=13 or s.GSTType=15) And ((t.maintype='SL' And (l.REASONCODE=0 or l.REASONCODE is null)) OR (t.MAINTYPE='PR' AND P.REASONCODE<>0)) ";
                    ...
                    mSQL += " And s.Branch in ('" + mbranchcode + "') " + (string.IsNullOrEmpty(mDocString) ? "" : " And CharIndex(t.Type,'" + mDocString + "')=0");
                ...
                case "b2cs":
                    ...
                    break;
                case "cdnr":
                    //mSQL += " ((s.SubType='RS' And l.ReasonCode<>0) " + (chkReturnSR == true ? "" : " or s.SubType='NS'") + " or s.SubType='AP' " + (chkReturnP == true ? "or s.SubType='NP'" : "") + ") And a.GSTNo<>''";
                    ...
                    //mSQL += " ((s.SubType='RS' And l.ReasonCode<>0) or s.SubType='NS') And a.GSTNo<>''";
                    morderby = "t.DocDate,t.Srl";
                    break;
                case "cdnur":
