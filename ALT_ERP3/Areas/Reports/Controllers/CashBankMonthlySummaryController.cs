...
{
    public class CashBankMonthlySummaryController : BaseController
    {
        ...
        public ActionResult GetAccountList(string term)
        {
            if (term == "")
            {
                var result = ctxTFAT.Master.Where(x => (x.BaseGr == "B" || x.BaseGr == "C") && x.Name.Contains(term)).Select(m => new { m.Code, m.Name }).OrderBy(n => n.Name).Take(10).ToList();
